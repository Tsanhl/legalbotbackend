...
from model_applicable_service import (
    ...
    is_interactive_codex_supervisor_handoff,
    sanitize_output_against_allowlist,
    strip_internal_reasoning
)
from legal_answer_quality_controls import build_sqe2_practice_marking_loop_block
try:
    from model_applicable_service import (
    ...
    _infer_subject_guide_slug = None
    _subject_guide_excerpt_for_query = None
...
BACKEND_COMPLETE_ANSWER_REGEN_MAX_ATTEMPTS = 2
...
SQE_RAG_SEARCH_POLICY_LINE = (
    "Use indexed RAG/source materials first and any backend search context supplied by the route; "
    "do not answer from memory where sources are available."
)
...
def build_sqe2_written_guide_prompt(
    ...
    *,
    ...
    return "\n".join([
        ...
        build_sqe2_practice_marking_loop_block(guide["label"]),
...
def build_sqe2_marking_prompt(
    *,
    ...
    return "\n".join([
        ...
        "11. End with `Next targeted practice`: one harder follow-up task recommendation based on the weakest criterion.",
        build_sqe2_practice_marking_loop_block(guide["label"]),
        "",
        "Question / task:",
...
def _backend_text_from_response(response: Any) -> str:
...
def _sqe2_task_response_needs_repair(text: str) -> bool:
    low = (text or "").lower()
    if _backend_word_count(text) < 220:
        return True
    required = ("candidate instructions", "client/matter facts", "specific task")
    if not all(item in low for item in required):
        return True
    refusal_markers = (
        "cannot be generated",
        "no client scenario",
...
def _sqe2_marking_response_needs_repair(text: str) -> bool:
    low = (text or "").strip().lower()
    if not low.startswith("sqe2 marking result"):
        return True
    if _backend_word_count(text) < 220:
        return True
    required = ("criterion", "corrected", "next targeted practice")
    return not all(item in low for item in required)


def _infer_sqe2_practice_area(enquiry: str, explicit: str = "") -> str:
    raw = f"{explicit or ''} {enquiry or ''}".lower()
    for area in SQE2_WRITTEN_PRACTICE_AREAS:
        if area.lower() in raw:
            return area
    if "civil" in raw or "dispute" in raw or "injunction" in raw or "part 36" in raw:
        return "Dispute Resolution"
    if "criminal" in raw or "police" in raw or "bail" in raw or "abh" in raw:
        return "Criminal Litigation"
    if "property" in raw or "lease" in raw or "completion" in raw:
        return "Property Practice"
    if "will" in raw or "probate" in raw or "estate" in raw:
        return "Wills and Intestacy, Probate Administration and Practice"
    if "company" in raw or "business" in raw or "director" in raw:
        return "Business Organisations, Rules and Procedures"
    return "Dispute Resolution"


def _build_sqe2_practical_task_repair(
    enquiry: str,
    *,
    ...
    guide = SQE2_WRITTEN_SKILL_GUIDES[selected_skill]
    count = max(1, min(int(question_count or _extract_backend_sqe_question_count(enquiry) or 1), 3))
    practice_area = _infer_sqe2_practice_area(" ".join([enquiry or "", focus or ""]))
    tasks: List[str] = []
    for task_no in range(1, count + 1):
        if selected_skill == "legal_research":
            body = "\n".join([
                f"Task {task_no} - Legal research / {practice_area} / urgent interim relief and settlement pressure",
                "",
                "Candidate instructions",
                f"You are a trainee solicitor. You have {guide['time']} to prepare a research note to the supervising partner. Use only the source extracts below and the facts given. Do not draft a client letter.",
                "",
                "Client/matter facts",
                "We act for Northbank Components Ltd, a supplier of precision parts. Its former sales director, Mara, left four weeks ago and joined a competitor. Since then, three major customers have moved to the competitor after receiving price lists that match Northbank's confidential discount schedule. Northbank's standard employment contract contains a six-month non-solicitation covenant, a confidentiality clause, and a clause requiring return of company devices immediately on termination. Mara returned her laptop late. IT found that, two days before leaving, she downloaded a customer margin spreadsheet to a personal cloud account.",
                "",
                "Today is Monday. Northbank wants urgent relief by Friday because a framework tender closes next week. The client also asks whether we can tell the competitor's solicitor that we have forensic proof Mara copied the customer list. The IT report says only that a file with a similar name was uploaded; it does not identify its contents.",
                "",
                "Documents/extracts",
                "Source 1 - Interim injunctions: the court considers whether there is a serious issue to be tried, whether damages would be an adequate remedy for either side, where the balance of convenience lies, and whether there are any special factors such as delay or clean hands.",
                "Source 2 - Springboard relief: equity may restrain a defendant from using confidential information or an unlawful head start obtained through misuse of such information. Relief must be proportionate and normally should not punish ordinary competition.",
                "Source 3 - Restrictive covenants: post-termination restraints are enforceable only if they protect a legitimate business interest and go no further than reasonably necessary as to duration, scope and restricted activity.",
                "Source 4 - Without-notice applications: full and frank disclosure is required. The applicant must draw the court's attention to material points against its case, including delay and evidential uncertainty.",
                "Source 5 - Professional conduct: a solicitor must not make allegations or assertions unless they are properly arguable and supported by available evidence. A solicitor must not mislead the court or another party.",
                "",
                "Specific task",
                "Prepare a research note for the partner answering:",
                "1. whether Northbank has a realistic basis for an interim injunction or springboard relief;",
                "2. how the restrictive covenant affects the application;",
                "3. what evidence gaps or timing risks matter most;",
                "4. what professional-conduct limits apply to what we say to the competitor's solicitor and, if urgent court relief is sought, to the court.",
                "",
                "Assessment traps",
                "There is no automatic injunction merely because a covenant exists. Separate confidentiality/springboard relief from non-solicitation relief. Address adequacy of damages, delay, proportionality, evidential uncertainty, and the ethics of overstating the forensic evidence.",
            ])
        elif selected_skill == "legal_writing":
            body = "\n".join([
                f"Task {task_no} - Legal writing / {practice_area} / client advice under pressure",
                "",
                "Candidate instructions",
                f"You have {guide['time']} to write a clear email to the client in the partner's name. The client is not a lawyer. Do not include citations.",
                "",
                "Client/matter facts",
                "Your client exchanged contracts to buy a warehouse for use as a new distribution unit. Completion was due yesterday. The client had mortgage funds and balance money ready, but the seller did not complete because a tenant has refused to leave part of the warehouse. The client still wants the property because its current lease expires in three weeks. It has already incurred removal, storage, temporary security, and bridging finance costs.",
                "",
                "The seller's solicitor says the tenant will leave within seven days and asks us to confirm that the client will not claim compensation if completion happens next week. The client is worried that serving a notice might make the seller withdraw from the sale. The client also asks whether we can promise the seller's solicitor today that the client will definitely complete next week if the seller becomes ready.",
                "",
                "Documents/extracts",
                "Extract 1 - The contract incorporates standard sale conditions. A party ready, able and willing to complete may serve a notice to complete after the contractual completion time has passed. The notice makes time of the essence and gives the defaulting party a final period to complete.",
                "Extract 2 - If completion is delayed by default, compensation may be payable at the contractual rate and the innocent party may also seek recoverable losses if properly evidenced.",
                "Extract 3 - If a party still wants the property, specific performance may be considered, but it is a court remedy involving cost, delay and litigation risk.",
                "Extract 4 - A solicitor must not give an undertaking or assurance unless authorised and able to perform it. A solicitor must not give away a client's compensation position without instructions.",
                "",
                "Specific task",
                "Write the client email explaining: whether the client has lost the purchase; what a notice to complete does; whether compensation or other losses may be claimed; whether specific performance may be available; what we should say to the seller's solicitor; and what evidence/instructions we need now.",
                "",
                "Assessment traps",
                "Use plain English, answer the client's real worry first, avoid over-promising recovery of losses, distinguish contractual compensation from wider losses, and do not give an undertaking or waive the client's compensation position without clear authority.",
            ])
        elif selected_skill == "legal_drafting":
            if practice_area == "Business Organisations, Rules and Procedures":
                body = "\n".join([
                    f"Task {task_no} - Legal drafting / {practice_area} / conflicted director transaction approval",
                    "",
                    "Candidate instructions",
                    f"You have {guide['time']} to draft the requested board minutes extract and shareholder written resolution only. Do not give advice or explain the law.",
                    "",
                    "Client/matter facts",
                    "We act for Alder Tech Ltd, a private company limited by shares with Model Articles. It has two directors, Iman and Joel. Iman owns 60% of the shares and Joel owns 40%. Alder proposes to buy specialist testing equipment for 92,000 pounds from Iman Analytics Ltd, a company wholly owned by Iman. Alder's latest accounts show net assets of 500,000 pounds. Joel has asked for the transaction to proceed only if the conflict is properly recorded and shareholder approval is obtained before completion.",
                    "",
                    "Iman says he should chair and vote at the board meeting because he owns the seller. Joel is willing to approve calling a shareholder decision but does not want the minutes to suggest that he has approved the price as fair without an independent valuation. The draft sale contract must be conditional on shareholder approval and completion must not occur until the approval is passed.",
                    "",
                    "Documents/extracts",
                    "Extract 1 - Model Articles require directors to declare interests and conflicted directors are generally not counted in quorum or voting on decisions affected by the conflict unless the articles or shareholders permit it.",
                    "Extract 2 - A substantial property transaction involving a director or connected person requires shareholder approval by ordinary resolution before the transaction is entered into, or the contract should be conditional on that approval.",
                    "Extract 3 - The board should resolve only procedural steps that non-conflicted directors can properly approve, including noting the conflict, approving circulation of the written resolution, and requiring independent valuation before completion.",
                    "",
                    "Specific task",
                    "Draft: 1. the operative board minutes extract recording the conflict, quorum/voting treatment, conditional approval route, valuation condition and authority to circulate the shareholder written resolution; and 2. a concise shareholder ordinary written resolution approving the transaction subject to the stated conditions.",
                    "",
                    "Assessment traps",
                    "Draft operative wording, not advice. Do not count Iman in conflicted board approval unless the facts permit it. Do not make Joel certify market value. Make the sale conditional on shareholder approval and valuation evidence. Avoid suggesting completion can happen before approval.",
                ])
            else:
                body = "\n".join([
                    f"Task {task_no} - Legal drafting / {practice_area} / urgent claim wording",
                    "",
                    "Candidate instructions",
                    f"You have {guide['time']} to draft the requested document extract only. Do not give advice.",
                    "",
                    "Client/matter facts",
                    "The claimant supplied specialist components for 12,000 pounds. The defendant accepted delivery, paid 2,000 pounds, and promised to pay the balance within seven days. The defendant has not paid and now alleges vaguely that some parts were late, but it has not identified defects or rejected the goods.",
                    "",
                    "Documents/extracts",
                    "Draft concise Particulars of Claim including contract formation, express terms, performance, breach, loss, interest, remedy, and statement of truth.",
                    "",
                    "Specific task",
                    "Draft the Particulars of Claim.",
                    "",
                    "Assessment traps",
                    "Use operative pleading language, plead facts not submissions, include the sum claimed and interest, and avoid adding unevidenced allegations.",
                ])
        else:
            body = "\n".join([
                f"Task {task_no} - Case and matter analysis / {practice_area} / risk and next steps",
                "",
                "Candidate instructions",
                f"You have {guide['time']} to prepare a note to the partner. Analyse issues, risks, options, professional conduct, and next steps.",
                "",
                "Client/matter facts",
                "We act for a private company with two directors/shareholders. One director caused the company to buy equipment from a business owned by his spouse. No board minutes or shareholder resolution can be found. The price appears materially above market value. The other director wants to challenge the transaction but asks whether we act for her personally or the company.",
                "",
                "Documents/extracts",
                "The company uses Model Articles. The latest accounts show net assets of 240,000 pounds. The purchase price was 75,000 pounds. Comparable equipment is advertised for about 50,000 pounds.",
                "",
                "Specific task",
                "Prepare the note covering approvals, director duties, conflict/quorum issues, remedies, client identity, and next steps.",
                "",
                "Assessment traps",
                "Do not treat majority control as solving director duties. Identify substantial-property-transaction risk, conflict, ratification limits, and who the solicitor acts for.",
            ])
        tasks.append(body)
    header = "\n".join([
        "[Backend SQE2 task repair output]",
        f"Exam track: {SQE_STAGE_LABELS.get(stage, 'SQE2')}",
        f"Selected written skill: {guide['label']}",
        f"Practice area: {practice_area}",
        "Answer policy: answer material is withheld for testing.",
        "",
    ])
    return header + "\n\n".join(tasks)


def _build_sqe2_marking_repair(
    *,
    question: str,
    candidate_answer: str,
    skill: Optional[str] = None,
    ...
    selected_skill = normalize_sqe2_written_skill(skill, combined)
    guide = SQE2_WRITTEN_SKILL_GUIDES[selected_skill]
    area = practice_area or _infer_sqe2_practice_area(combined)
    low_question = (question or "").lower()
    spouse_abh = "spouse" in low_question and ("abh" in low_question or "assault" in low_question)
    property_completion = selected_skill == "legal_writing" and (
        "completion" in low_question or "notice to complete" in low_question
    )
    business_director = selected_skill == "case_matter_analysis" and (
        "director" in low_question or "shareholder" in low_question or "model articles" in low_question
    )
    if spouse_abh:
        overall_grade = "E - clearly unsatisfactory / below pass-level"
        criteria_lines = [
            "- Relevant facts / task focus: D = 2/5. The answer identifies the spouse evidence issue, but it does not distinguish the key statutory categories or the client's professional-conduct risk.",
            "- Structure and communication: D = 2/5. The answer is short and direct, but it is conclusory and does not give a partner usable research advice.",
            "- Correct application of law: E = 1/5. It reverses the compellability rule, treats hearsay as automatic, and gives unsafe witness-contact advice.",
            "- Comprehensive application, ethics and next steps: F = 0/5. The suggested client contact with the spouse is dangerous and should be corrected immediately.",
        ]
        skills_subtotal = "4/10"
        law_subtotal = "1/10"
        corrected = "\n".join([
            "The spouse is competent to give evidence for the prosecution, but competence is separate from compellability. A spouse is compellable for the prosecution only in specified categories, including where the offence involves assault, injury or threat of injury to the spouse/civil partner or to a person under 16. Here the alleged ABH is against a neighbour, not against the spouse and not against a child under 16 on the facts given. The better conclusion is therefore that the spouse is competent but probably not compellable.",
            "",
            "The earlier police statement should not be treated as automatically admissible. The prosecution would need to identify a valid hearsay gateway and the court would still consider whether admission is in the interests of justice. Mere unwillingness to attend is not enough by itself.",
            "",
            "The client must not be advised to speak to the spouse to discourage attendance or change evidence. That would create serious professional-conduct and criminal-risk issues. We should take instructions, warn the client not to contact or influence the witness, and prepare to respond to any prosecution hearsay application.",
        ])
        next_practice = "Next targeted practice: complete a harder legal research task on witness compellability, hearsay gateways, and professional conduct where the client asks to influence a witness."
    elif property_completion:
        overall_grade = "D/E - borderline to clearly unsatisfactory / likely below pass-level"
        criteria_lines = [
            "- Relevant facts / recipient focus: D = 2/5. The answer spots seller default but does not answer the client's immediate fear about losing the purchase and does not mention the seller's failure to complete in practical terms.",
            "- Structure and plain English: C/D = 2/5. It is short and understandable, but too skeletal for a client email and lacks headings, reassurance, options, and concrete next steps.",
            "- Correct application of law: D = 2/5. It mentions breach and court action but misses notice to complete, time becoming of the essence, compensation for delayed completion, and the realistic role of specific performance.",
            "- Comprehensive application, ethics and next steps: E = 1/5. It over-promises 'all your costs', does not ask for evidence of losses, and does not address authority before waiving compensation or giving assurances to the seller's solicitor.",
        ]
        skills_subtotal = "4/10"
        law_subtotal = "3/10"
        corrected = "\n".join([
            "Dear [Client],",
            "",
            "You have not lost the purchase simply because the seller failed to complete yesterday. Contracts have been exchanged, so both parties remain bound. Because you were ready to complete and the seller was not, the seller is in breach.",
            "",
            "The usual next step is for us to serve a notice to complete. That gives the seller a final period to complete and makes time of the essence. If the seller still does not complete after that period, you would then have stronger remedies, including ending the contract and claiming recoverable losses. If you still want the property, we can also consider specific performance, which is a court order requiring the seller to complete, but that involves cost and delay.",
            "",
            "You may be entitled to contractual compensation and possibly other losses caused by the delay, such as reasonable storage or removal costs, if properly evidenced. Please send invoices and receipts. We should not confirm that you will waive compensation unless you expressly instruct us to do so after advice.",
            "",
            "I recommend that we contact the seller's solicitors immediately, reserve your rights, prepare a notice to complete, and ask for a firm timetable for vacant possession.",
        ])
        next_practice = "Next targeted practice: write a harder client email on delayed completion where the other side seeks an undertaking or waiver of compensation."
    elif business_director:
        overall_grade = "E - clearly unsatisfactory / below pass-level"
        criteria_lines = [
            "- Relevant facts / task focus: D = 2/5. The answer notices unfair price but misses the key facts: Model Articles, connected seller, 90,000 pound value, net assets, absent minutes, absent shareholder approval, and client identity.",
            "- Structure and partner usefulness: D = 2/5. It is too brief for case and matter analysis and gives no issue map, risk analysis, remedies, or document requests.",
            "- Correct application of law: E = 1/5. Majority shareholding does not cure directors' duties, conflict rules, quorum problems, or substantial-property-transaction approval. Retrospective approval is not a simple answer.",
            "- Comprehensive application, ethics and next steps: E = 1/5. It fails to address who the firm acts for, conflict between company and directors, valuation evidence, possible voidability/account of profit, or unfair prejudice risk.",
        ]
        skills_subtotal = "4/10"
        law_subtotal = "2/10"
        corrected = "\n".join([
            "The transaction is vulnerable. The equipment purchase appears to involve a director-connected seller, a conflicted director, and a value of 90,000 pounds against net assets of 300,000 pounds. That is likely to trigger substantial-property-transaction approval by ordinary resolution, or the contract should have been conditional on such approval.",
            "",
            "The director also had a conflict and should not be treated as solving the board decision merely because they control shares. Under Model Articles, conflict may affect quorum and voting. The absence of board minutes and shareholder approval is therefore a serious procedural and substantive risk.",
            "",
            "Possible consequences include the company seeking to avoid the transaction, recover loss, obtain an account of profit, or require indemnity if the price was excessive. The other director may also have unfair prejudice concerns if control was used to benefit the conflicted director's family company.",
            "",
            "We must clarify client identity. If we act for the company, we do not act personally for the complaining director. We should review articles, board papers, the sale contract, valuation evidence, shareholder resolutions, and any communications before advising on challenge, ratification, or settlement.",
        ])
        next_practice = "Next targeted practice: complete a harder case and matter analysis task on a conflicted director transaction with ratification, unfair prejudice, and client-identity issues."
    else:
        overall_grade = "D/E - likely below pass-level"
        criteria_lines = [
            "- Relevant facts / task focus: D = 2/5. The answer engages with the broad task but omits material facts and does not tailor advice to the requested output.",
            "- Structure and communication: D = 2/5. The answer is understandable but too brief and lacks the issue-by-issue structure expected for this SQE2 written skill.",
            f"- Correct application of law: D = 2/5. The answer does not show enough applied legal analysis for {guide['label']} and leaves important legal consequences uncertain.",
            "- Comprehensive application, ethics and next steps: E = 1/5. It misses missing-information points, professional-conduct risks, and practical next steps.",
        ]
        skills_subtotal = "4/10"
        law_subtotal = "3/10"
        corrected = "\n".join([
            "A high-scoring answer should answer the exact task first, separate each legal issue, apply the provided facts and sources, identify missing information, and finish with practical next steps. It should avoid unsupported assertions and should flag any professional-conduct issue even if the question does not label it.",
            "",
            f"For {guide['label']}, the answer should follow this shape: {guide['answer_shape']}",
        ])
        next_practice = f"Next targeted practice: complete a harder {guide['label']} task in {area} focusing on the weakest criterion above."
    return "\n".join([
        "SQE2 marking result",
        "",
        f"Skill marked: {guide['label']}",
        f"Practice area: {area}",
        f"Overall simulated grade: {overall_grade}",
        "",
        "Criterion assessment",
        *criteria_lines,
        "",
        f"Skills subtotal: {skills_subtotal}",
        f"Application of law subtotal: {law_subtotal}",
        f"Overall simulated grade: {overall_grade}. This is below pass-level or unsafe unless the missing law, structure and practical advice are corrected.",
        "",
        "Corrected high-scoring answer outline",
        corrected,
        "",
        next_practice,
    ])
...
def _maybe_reinforce_law_medicine_statutory_routes(answer_text: str, prompt_text: str) -> str:
    """
    Keep course-bound Law and Medicine answers from drifting into abstract
    autonomy ethics without naming the legal route being criticised.
    """
    prompt_low = (prompt_text or "").lower()
    if "law and medicine" not in prompt_low:
        return answer_text
    if not any(marker in prompt_low for marker in ("module syllabus", "within the syllabus", "stay within", "course-bound")):
        return answer_text
    answer_low = (answer_text or "").lower()
    needs_course_label = not any(marker in answer_low for marker in ("course-bound", "module syllabus", "within the syllabus"))
    needs_route_note = not any(term in answer_low for term in ("mca 2005", "mental capacity act 2005", "human tissue act 2004", "abortion act 1967", "human fertilisation and embryology act 1990", "hfea 1990"))
    abortion_prompt = any(marker in prompt_low for marker in ("abortion", "fetal abnormality", "foetal abnormality"))
    needs_abortion_route_note = abortion_prompt and not any(marker in answer_low for marker in ("s.1(1)(a)", "section 1(1)(a)", "social ground"))
    if not needs_course_label and not needs_route_note and not needs_abortion_route_note:
        return answer_text
    notes: List[str] = []
    if needs_course_label:
        notes.append(
            "Course-bound mode: this answer stays within the Law and Medicine module syllabus and uses only the requested syllabus examples."
        )
    if needs_route_note:
        notes.append(
            "The legal route matters before the ethical critique. Consent and refusal start from common-law bodily integrity and, where capacity is in issue, the Mental Capacity Act 2005. "
            "End-of-life treatment disputes commonly require the Mental Capacity Act 2005 route for incapacitous patients and the Human Rights Act 1998 where rights arguments are raised. "
            "Transplantation and post-death bodily use should be tied to the Human Tissue Act 2004 consent framework before evaluating whether autonomy is being respected or reduced to administrative permission."
        )
    if needs_abortion_route_note:
        notes.append(
            "Abortion route note: fetal-abnormality reform questions should expressly compare the Abortion Act 1967 s.1(1)(a) social/health ground, which is the ordinary early-pregnancy route, with s.1(1)(d), the fetal-abnormality ground. The critique should then ask whether a separate disability-based late route is justified, discriminatory, or better replaced by a woman-centred late-access ground."
        )
    route_note = "\n".join(notes)
    ...
    if "\n\n" in text:
        ...
        return "\n\n".join([first, route_note, rest]).strip()
    return "\n\n".join([route_note, text]).strip()
...
def _sqe_enquiry_requests_answers(enquiry: str) -> bool:
    ...
    if any(token in low for token in [
        "do not give the model answer",
        "do not give model answer",
        "do not include the model answer",
    ...
    *,
    ...
    if stage == "sqe2":
        ...
        return "\n".join([
            ...
            SQE_RAG_SEARCH_POLICY_LINE,
            build_sqe2_practice_marking_loop_block(skill_guide["label"]),
...
def _split_sqe1_question_blocks(text: str) -> List[str]:
    src = str(text or "").strip()
    if not src:
        return []
    matches = list(re.finditer(r"(?im)^\s*Question\s+\d+\s*(?:[-–:]\s*[^\n]+)?\s*$", src))
    blocks: List[str] = []
    for idx, match in enumerate(matches):
        end = matches[idx + 1].start() if idx + 1 < len(matches) else len(src)
        block = src[match.start():end].strip()
        if block:
            blocks.append(block)
    return blocks


def _sqe1_question_set_issues(
    text: str,
    *,
    ...
    """
    Deterministic SQE1/FLK validator.

    This deliberately checks only format risks that can be verified without an
    official answer key: question count, A-E uniqueness, answer-key uniqueness
    where answers were requested, and broad option-length parity.
    """
    txt = str(text or "").strip()
    issues: List[str] = []
    if not txt:
        return ["SQE question-set output is empty."]
    if re.search(r"(?im)^\s*Part\s+[IVXLCDM]+\s*:", txt):
        issues.append("SQE question-set output used essay/problem Part headings.")

    blocks = _split_sqe1_question_blocks(txt)
    if not blocks:
        issues.append("SQE question-set output has no `Question N - ...` blocks.")
        return issues
    if isinstance(expected_count, int) and expected_count > 0 and len(blocks) != expected_count:
        issues.append(f"SQE question-set output has {len(blocks)} question blocks; expected {expected_count}.")

    for q_idx, block in enumerate(blocks, start=1):
        option_matches = list(
            re.finditer(
                r"(?ims)^\s*([A-E])\.\s+(.+?)(?=^\s*[A-E]\.\s+|^\s*Question\s+\d+\s*(?:[-–:]|\b)|\Z)",
                block,
            )
        )
        labels = [m.group(1).upper() for m in option_matches]
        if labels != ["A", "B", "C", "D", "E"]:
            issues.append(f"Question {q_idx} must contain exactly one A-E option sequence.")
            continue
        option_lengths = [len(re.findall(r"\b[\w'-]+\b", m.group(2) or "")) for m in option_matches]
        if option_lengths:
            min_len = max(1, min(option_lengths))
            max_len = max(option_lengths)
            if max_len >= 18 and max_len / min_len > 2.75:
                issues.append(
                    f"Question {q_idx} option parity weak: longest option is too much longer than shortest."
                )
        if wants_answers:
            answers = re.findall(r"(?im)\b(?:correct\s+answer|answer)\s*[:\-]\s*([A-E])\b", block)
            if len(answers) != 1:
                issues.append(f"Question {q_idx} must identify exactly one correct answer when answers are requested.")

    return issues[:12]


def _build_sqe1_question_set_repair_prompt(
    *,
    original_prompt: str,
    issues: List[str],
    previous_output: str,
    ...
    return "\n\n".join([
        original_prompt,
        "[SQE QUESTION-SET VALIDATOR RETRY]",
        "Regenerate the SQE question set only. Do not write an essay or problem answer.",
        "Fix every deterministic validator issue below:",
        issue_lines,
        "Hard format rules: each question must have exactly one `Question N - ...` heading, one `Question` stem label, and one A-E option sequence with options of broadly similar length. If answers are requested, each question must identify exactly one correct answer.",
        "Previous invalid output excerpt for diagnosis only:",
        str(previous_output or "").strip()[:5000],
    ]).strip()
...
def send_sqe_question_set_with_docs(
    ...
    if stage != "sqe2":
        expected_count = int(question_count or _extract_backend_sqe_question_count(enquiry) or SQE_FLK_DEFAULT_QUESTION_COUNT)
        wants_answers = _sqe_enquiry_requests_answers(enquiry) if include_answers is None else bool(include_answers)
        sqe_issues = _sqe1_question_set_issues(
            _backend_text_from_response(response),
            expected_count=expected_count,
            wants_answers=wants_answers,
        )
        if sqe_issues:
            repair_prompt = _build_sqe1_question_set_repair_prompt(
                original_prompt=prompt,
                issues=sqe_issues,
                previous_output=_backend_text_from_response(response),
            )
            response, rag_context = send_complete_answer_with_docs(
                api_key=api_key,
    ...
    if stage == "sqe2" and _sqe2_task_response_needs_repair(_backend_text_from_response(response)):
        repaired = _build_sqe2_practical_task_repair(
            enquiry,
            exam_stage=stage,
            question_count=question_count,
            focus=focus,
        )
        return repaired, rag_context
...
def send_sqe2_marking_with_docs(
    ...
    if (
        ...
        and _backend_text_from_response(response).strip().lower() == "sqe2 marking result"
    ):
        return response, rag_context
    if _sqe2_marking_response_needs_repair(_backend_text_from_response(response)):
        repaired = _build_sqe2_marking_repair(
            question=question,
            candidate_answer=candidate_answer,
            skill=skill,
            practice_area=practice_area,
...
def _looks_like_backend_complete_answer_request(
    ...
    if any(marker in low for marker in legal_answer_markers):
        return True
    if re.search(r"\bq(?:uestion)?\s*\d+\b", low) and any(
        marker in low
        for marker in (
            "advise", "essay", "problem", "critically", "discuss", "evaluate",
...
def _count_answer_words_for_window(answer_text: str) -> int:
    """
    Count substantive answer words for strict target windows.

    Required delivery markers such as "(End of Answer)" should not make an
    otherwise on-target answer fail the word cap.
    """
    cleaned = re.sub(r"(?im)^\s*\(End of Answer\)\s*$", "", answer_text or "")
    cleaned = re.sub(r"(?im)^\s*Will Continue to next part, say continue\s*$", "", cleaned)
    return _count_words(cleaned)
...
def _has_terminal_inline_authority_parenthetical(text: str) -> bool:
    """
    True only where the supporting authority parenthetical is the terminal support
    for the sentence/proposition: sentence (authority).
    """
    stripped = (text or "").strip()
    if not stripped:
        return False
    for match in _INLINE_AUTHORITY_PAREN_RE.finditer(stripped):
        if not _INLINE_AUTHORITY_SIGNAL_RE.search(match.group(0)):
            continue
        tail = stripped[match.end():].strip()
        if re.fullmatch(r"[.;:!?\"'’”\]\)]*", tail or "."):
            return True
    return False
...
def _complete_answer_sentence_support_issues(answer_text: str) -> List[str]:
    ...
    def _has_inline_support(s: str) -> bool:
        stripped = (s or "").strip()
        if _has_terminal_inline_authority_parenthetical(stripped):
            return True
        for match in harvard_pat.finditer(stripped):
            tail = stripped[match.end():].strip()
            if re.fullmatch(r"[.;:!?\"'’”\]\)]*", tail or "."):
                return True
        return False
    ...
    return [
        ...
        f"{len(missing)} argumentative sentence(s) lack terminal inline authority support in the pattern sentence (citation). "
        f"Examples: {preview}"
    ]


def _has_any_text(low_text: str, terms: Tuple[str, ...]) -> bool:
    return any(term in low_text for term in terms)


_GUIDE_CONTRACT_STOPWORDS = {
    "about", "above", "actual", "advice", "advise", "against", "analysis",
    "answer", "answers", "apply", "asks", "authority", "available", "because",
    "before", "between", "briefly", "cannot", "clear", "conclude", "conclusion",
    "consider", "court", "directly", "distinct", "does", "each", "essay",
    "exact", "explain", "facts", "first", "focus", "from", "general", "guide",
    "identify", "issue", "issues", "keep", "legal", "likely", "make", "must",
    "need", "only", "point", "points", "problem", "question", "rather",
    "relevant", "remedy", "route", "rule", "separate", "should", "state",
    "test", "than", "that", "their", "them", "then", "these", "this", "those",
    "under", "unless", "where", "whether", "which", "with", "without",
}


def _guide_contract_keywords(text: str, *, limit: int = 8) -> List[str]:
    words = []
    for raw in re.findall(r"\b[a-z][a-z0-9\-]{3,}\b", (text or "").lower()):
        tok = raw.strip("-")
        if not tok or tok in _GUIDE_CONTRACT_STOPWORDS:
            continue
        if tok not in words:
            words.append(tok)
        if len(words) >= limit:
...
def _guide_contract_term_hit(low_text: str, term: str) -> bool:
    raw = (term or "").strip().lower()
    if not raw:
        return False
    cleaned = re.sub(r"\s+", " ", raw)
    if " " in cleaned or any(ch.isdigit() for ch in cleaned):
        return cleaned in low_text
    return bool(re.search(rf"\b{re.escape(cleaned)}\b", low_text))


def _topic_profile_guide_contract_issues(
    answer_text: str,
    prompt_text: str,
) -> List[str]:
    """
    Generic enforcement for the topic/profile guides embedded in the backend.

    The prompt layer can expose hundreds of subject rules, but without a
    deterministic guard a shallow answer can still pass after merely naming the
    topic. This gate checks that the final answer has at least a minimal
    footprint from the detected profile's expected keywords and issue-bank.
    """
    if _infer_retrieval_profile is None:
        return []
    prompt_low = (prompt_text or "").lower()
    if not (
        _is_problem_flow(prompt_text, [])
        or _is_essay_flow(prompt_text, [])
        or "problem question" in prompt_low
        or "essay question" in prompt_low
        or "advise" in prompt_low
    ):
        return []
    try:
        profile = _infer_retrieval_profile(prompt_text or "")
    ...
    topic = str((profile or {}).get("topic") or "").strip()
    if not topic or topic == "general_legal":
        return []

    answer_low = (answer_text or "").lower()
    if not answer_low.strip():
        return []

    expected_terms = [
        str(term or "").strip()
        for term in (profile.get("expected_keywords") or [])[:12]
        if str(term or "").strip()
    ]
    expected_hits = sum(1 for term in expected_terms if _guide_contract_term_hit(answer_low, term))
    expected_need = min(4, max(2, len(expected_terms) // 3)) if len(expected_terms) >= 4 else 0

    issue_items = [
        str(item or "").strip()
        for item in (profile.get("issue_bank") or [])[:6]
        if str(item or "").strip()
    ]
    issue_failures: List[str] = []
    issue_passes = 0
    for item in issue_items:
        kws = _guide_contract_keywords(item, limit=8)
        if not kws:
            continue
        hits = sum(1 for kw in kws if _guide_contract_term_hit(answer_low, kw))
        needed = 1 if len(kws) <= 3 else 2
        if hits >= needed:
            issue_passes += 1
        else:
            issue_failures.append(item)

    issue_need = min(2, max(1, len(issue_items) // 3)) if len(issue_items) >= 2 else 0
    expected_failed = bool(expected_need and expected_hits < expected_need)
    issue_failed = bool(issue_need and issue_passes < issue_need)

    if not (expected_failed and issue_failed):
        return []

    detail_parts = []
    if expected_terms:
        detail_parts.append(f"expected keyword coverage {expected_hits}/{len(expected_terms)}")
    if issue_failures:
        detail_parts.append("missing issue-guide moves: " + "; ".join(issue_failures[:3]))
    return [
        f"Topic guide contract under-covered for {topic}: "
        + "; ".join(detail_parts)
    ]


def _law_medicine_surgical_consent_methodology_issues(
    answer_text: str,
    prompt_text: str,
) -> List[str]:
    """
    Targeted deterministic gate for the high-risk Law and Medicine surgical
    consent pattern: disclosed/undisclosed risk plus an additional procedure
    outside the consented body part.
    """
    prompt_low = (prompt_text or "").lower()
    if not (
        ("law and medicine" in prompt_low or "medical law" in prompt_low)
        and _has_any_text(prompt_low, ("problem question", "advise", "advise rina", "advise the hospital"))
        and _has_any_text(prompt_low, ("right hand", "right-hand", "left wrist", "left-wrist", "fine motor", "dexterity", "pianist"))
        and _has_any_text(prompt_low, ("consent", "material risk", "montgomery", "battery", "negligence"))
    ):
        return []
    ...
    missing: List[str] = []
    if not (
        "montgomery" in answer_low
        and _has_any_text(answer_low, ("material risk", "material risks"))
        and _has_any_text(answer_low, ("reasonable person", "patient's position", "patient’s position", "particular patient"))
    ):
        missing.append("Montgomery material-risk test with the patient-specific limb")

    if not (
        _has_any_text(answer_low, ("pianist", "profession", "career", "fine motor", "dexterity"))
        and _has_any_text(answer_low, ("significance", "significant", "material"))
    ):
        missing.append("Rina's profession/personal circumstances as the reason a small risk is material")

    if not (
        _has_any_text(answer_low, ("alternative", "alternatives", "delay", "non-surgical", "non surgical", "pain management"))
        and _has_any_text(answer_low, ("mcculloch", "clinical judgment", "professional judgment", "bolam", "bolitho"))
    ):
        missing.append("reasonable alternatives and clinical judgment after Montgomery/McCulloch")

    if not ("battery" in answer_low and "negligence" in answer_low):
        missing.append("battery/negligence distinction")

    if not (
        _has_any_text(answer_low, ("left wrist", "left-wrist"))
        and _has_any_text(answer_low, ("scope of consent", "outside consent", "without consent", "unauthorised", "unauthorized", "battery"))
        and _has_any_text(answer_low, ("necessity", "urgent", "emergency", "life-threatening", "life threatening", "not urgent"))
    ):
        missing.append("left-wrist scope of consent and narrow necessity analysis")

    if not (
        "causation" in answer_low
        and _has_any_text(answer_low, ("chester", "but for", "would have refused", "would have delayed", "would have postponed"))
        and _has_any_text(answer_low, ("loss of autonomy", "autonomy", "loss of chance", "gregg", "shaw v kovac"))
    ):
        missing.append("causation, autonomy, and loss-of-chance limits")

    if not (
        _has_any_text(answer_low, ("damages", "remedy", "remedies", "injunction", "declaration"))
        and _has_any_text(answer_low, ("earnings", "income", "career", "pianist", "professional loss", "loss of profits"))
        and _has_any_text(answer_low, ("defence", "defenses", "defences", "clinically reasonable", "therapeutic exception", "trust"))
    ):
        missing.append("recoverable loss, professional earnings, remedies, and Trust defences")

    if not missing:
        return []
    return [
        "Law and Medicine surgical-consent methodology under-covered: "
        + "; ".join(missing[:6])
    ]


def _law_medicine_mixed_medical_decision_route_issues(
    answer_text: str,
    prompt_text: str,
) -> List[str]:
    """
    Targeted deterministic gate for mixed Law and Medicine hospital problem
    questions combining adult refusal/capacity, child treatment refusal,
    living organ donation, and ventilator/advance-decision facts.

    This pattern is high-risk because a fluent answer can mention autonomy and
    best interests while missing one of the mandatory statutory routes.
    """
    prompt_low = (prompt_text or "").lower()
    if not (
        ("law and medicine" in prompt_low or "medical law" in prompt_low or "hospital" in prompt_low)
        and _has_any_text(prompt_low, ("problem question", "advise", "advise the royal", "advise the hospital"))
    ):
        return []

    adult_refusal_prompt = (
        _has_any_text(prompt_low, ("adult", "patient", "aged", "capacity"))
        and _has_any_text(prompt_low, ("refuse", "refuses", "refusal", "declines"))
        and _has_any_text(prompt_low, ("surgery", "treatment", "operation", "life-threatening", "life threatening"))
    )
    child_treatment_prompt = (
        _has_any_text(prompt_low, ("child", "minor", "son", "daughter", "fourteen", "14", "sixteen", "16"))
        and _has_any_text(prompt_low, ("treatment", "medical", "surgery", "plaster", "consent", "refuse", "refuses", "refusal"))
    )
    living_donor_prompt = (
        _has_any_text(prompt_low, ("organ", "kidney", "transplant", "donor", "donation", "dialysis"))
        and _has_any_text(prompt_low, ("living", "child", "minor", "sixteen", "16", "family", "relative"))
    )
    eol_prompt = (
        _has_any_text(prompt_low, ("ventilator", "life-sustaining", "life sustaining", "coma", "withdraw", "withdrawal"))
        or _has_any_text(prompt_low, ("advance decision", "advance refusal", "advance directive", "written refusal"))
    )
    if sum(bool(x) for x in (adult_refusal_prompt, child_treatment_prompt, living_donor_prompt, eol_prompt)) < 3:
        return []
    ...
    if adult_refusal_prompt and not (
        _has_any_text(answer_low, ("adult refusal", "capacitous adult", "adult patient", "refusal"))
        and _has_any_text(answer_low, ("mental capacity act", "mca 2005", "section 1", "s 1", "ss 1", "ss 1-3"))
        and _has_any_text(answer_low, ("section 2", "s 2", "section 3", "s 3", "ss 2", "ss 3", "ss 1-3", "understand", "retain", "use or weigh", "communicate"))
        and _has_any_text(answer_low, ("unwise", "irrational", "re c", "re b", "re t"))
        and _has_any_text(answer_low, ("missing", "validate", "assess", "capacity assessment", "voluntariness", "belief"))
    ):
        missing.append("adult refusal: use the MCA capacity test, unwise-decision distinction, Re C/Re B/Re T route, and hospital validation of missing capacity/voluntariness facts")

    if child_treatment_prompt and not (
        _has_any_text(answer_low, ("child", "minor", "young person", "parental"))
        and _has_any_text(answer_low, ("gillick", "family law reform act", "children act", "parental responsibility", "court"))
        and _has_any_text(answer_low, ("welfare", "best interests", "override", "re w", "e v northern", "necessity"))
        and _has_any_text(answer_low, ("parental refusal", "parent's refusal", "parent’s refusal", "not decisive", "not determinative", "one parent"))
    ):
        missing.append("child treatment refusal: separate Gillick/FLRA/parental consent from welfare/court override and explain that parental refusal is not decisive")

    if living_donor_prompt and not (
        _has_any_text(answer_low, ("kidney", "organ", "donor", "donation", "transplant"))
        and _has_any_text(answer_low, ("human tissue act", "hta 2004", "section 33", "s 33", "ss 33", "code f", "independent assessor"))
        and _has_any_text(answer_low, ("court", "declaration", "approval", "high court", "court of protection"))
        and _has_any_text(answer_low, ("best interests", "voluntariness", "pressure", "no reward", "re y", "mc"))
        and _has_any_text(answer_low, ("sixteen", "16", "child donor", "minor", "family law reform act"))
    ):
        missing.append("living child organ donation: cover HTA 2004/Code F approval, court scrutiny for a minor donor, voluntariness/no reward, and best-interests/non-therapeutic donation authorities")

    if eol_prompt and not (
        _has_any_text(answer_low, ("ventilator", "life-sustaining", "life sustaining", "coma", "withdrawal", "withdraw"))
        and _has_any_text(answer_low, ("advance decision", "advance refusal", "mca 2005", "section 24", "s 24", "section 25", "s 25", "section 26", "s 26", "ss 24", "ss 24-26"))
        and _has_any_text(answer_low, ("valid", "validity", "applicable", "applicability", "life-sustaining", "life sustaining"))
        and _has_any_text(answer_low, ("signed", "witness", "written", "risk to life", "specified treatment"))
        and _has_any_text(answer_low, ("pressure", "voluntariness", "re t", "family", "partner"))
        and _has_any_text(answer_low, ("best interests", "aintree", "bland", "court", "continue ventilation", "declaration"))
    ):
        missing.append("life-sustaining treatment: analyse contemporaneous refusal, voluntariness/pressure, MCA ss 24-26 validity/applicability/formalities, the written refusal, best interests, and court/continue-treatment next step")

    if not (
        _has_any_text(answer_low, ("there are 3 issues", "three issues", "3 issues"))
        and _has_any_text(answer_low, ("issue 1", "issue one"))
        and _has_any_text(answer_low, ("issue 2", "issue two"))
        and _has_any_text(answer_low, ("issue 3", "issue three"))
        and _has_any_text(answer_low, ("hospital should", "hospital must", "advise the hospital", "trust should", "trust must"))
    ):
        missing.append("structure/practical advice: answer must expressly use the requested three-issue structure and advise the hospital on what to do next")

    if not missing:
        return []
    return [
        "Law and Medicine mixed medical-decision methodology under-covered: "
        + "; ".join(missing[:6])
    ]


def _subject_slug_for_prompt(prompt_text: str) -> str:
    if _infer_retrieval_profile is None:
        return ""
    try:
        profile = _infer_retrieval_profile(prompt_text or "") or {}
    except Exception:
        profile = {}
    topic = str(profile.get("topic") or "")
    if callable(_infer_subject_guide_slug):
        try:
            return str(_infer_subject_guide_slug(topic, prompt_text or "") or "")
        except Exception:
            return ""
    return topic


def _answer_has_any_term(answer_low: str, terms: Tuple[str, ...]) -> bool:
    return any(_guide_contract_term_hit(answer_low, term) for term in terms)


def _subject_route_prompt_scopes(prompt_text: str) -> List[str]:
    scopes = [prompt_text or ""]
    if callable(_extract_units_with_text):
        try:
            units = _extract_units_with_text(prompt_text or "") or []
        except Exception:
            units = []
        for unit in units:
            unit_text = str((unit or {}).get("text") or "").strip()
            if unit_text and unit_text not in scopes:
                scopes.append(unit_text)
    return scopes


def _subject_specific_problem_route_issues(answer_text: str, prompt_text: str) -> List[str]:
    """
    Per-subject deterministic route gates for high-risk problem answers.

    These gates do not replace RAG/guide prompting. They catch the recurring
    failure mode where an answer is fluent but misses the subject's mandatory
    route sequence.
    """
    answer_low = (answer_text or "").lower()
    if not answer_low.strip():
    ...
    issues: List[str] = []
    for scope in _subject_route_prompt_scopes(prompt_text):
        scope_low = (scope or "").lower()
        if not (
            "problem question" in scope_low
            or re.search(r"\badvise\b", scope_low)
            or "admissibility" in scope_low
            or "judicial review" in scope_low
        ):
            continue
        slug = _subject_slug_for_prompt(scope)
        missing: List[str] = []

        if slug == "contract_law":
            conditional_moves = [
                (
                    ("statement", "statements", "representation", "negotiation", "negotiations"),
                    ("term", "terms", "representation", "representations", "collateral warranty", "mere opinion"),
                    "term/representation/collateral-warranty classification",
                ),
                (
                    ("misrepresentation", "false", "inducement", "non-reliance", "non reliance"),
                    ("misrepresentation", "misrepresentation act", "inducement", "rescission", "section 2(1)", "s 2(1)"),
                    "misrepresentation gateway and remedy",
                ),
                (
                    ("breach", "failed", "failure", "non-performance", "non performance"),
                    ("breach", "condition", "warranty", "innominate", "reasonable care and skill"),
                    "breach/term classification",
                ),
                (
                    ("entire agreement", "non-reliance", "non reliance", "exclusion", "exclude", "excluding"),
                    ("entire agreement", "non-reliance", "non reliance", "exclusion clause", "ucta", "misrepresentation act 1967", "section 3", "s 3"),
                    "entire agreement, non-reliance and exclusion/statutory-control analysis",
                ),
                (
                    ("investor", "funding", "profit", "business opportunity", "goodwill", "consequential"),
                    ("remoteness", "hadley", "loss of chance", "business opportunity", "loss of profit", "causation", "proof"),
                    "remoteness, causation and proof of lost funding/business opportunity",
                ),
                (
                    ("rescission", "affirmation", "set-off", "set off", "remedies", "damages"),
                    ("rescission", "affirmation", "damages", "set-off", "set off", "mitigation", "price reduction"),
                    "remedies, bars and set-off",
                ),
            ]
        elif slug == "public_law":
            conditional_moves = [
                (
                    ("guidance", "secretary of state", "council", "local authority"),
                    ("power source", "statutory power", "guidance", "ultra vires", "legal status", "amenability"),
                    "legal status of guidance and power source",
                ),
                (
                    ("binding", "fetter", "fettered", "treated the guidance as binding"),
                    ("fetter", "fettering", "discretion", "binding", "policy"),
                    "fettering of discretion",
                ),
                (
                    ("no individual assessment", "refusal letter", "reasons", "unsuitable", "risk score"),
                    ("procedural fairness", "reasons", "individual assessment", "relevant consideration", "irrelevant consideration"),
                    "procedural fairness, reasons and individual assessment",
                ),
                (
                    ("algorithm", "risk score", "automated"),
                    ("algorithm", "risk score", "automated", "transparency", "explainability", "data protection"),
                    "algorithmic risk-score transparency/fairness",
                ),
                (
                    ("charter", "promise", "promising", "legitimate expectation"),
                    ("legitimate expectation", "clear", "unambiguous", "reliance", "fairness", "overriding public interest"),
                    "legitimate expectation route",
                ),
                (
                    ("article 10", "article 11", "echr", "protest", "march", "demonstration"),
                    ("article 10", "article 11", "proportionality", "legitimate aim", "necessity", "fair balance"),
                    "Articles 10/11 and proportionality",
                ),
                (
                    ("remedy", "remedies", "interim", "quashing", "declaration"),
                    ("quashing", "declaration", "interim relief", "mandatory order", "remittal", "remedy"),
                    "JR remedies and interim relief",
                ),
            ]
        elif slug == "evidence_law":
            conditional_moves = [
                (
                    ("cctv", "eyewitness", "identified", "identification", "social media", "masked"),
                    ("identification", "turnbull", "code d", "dock identification", "warning", "mistaken identification"),
                    "identification/CCTV reliability and warning route",
                ),
                (
                    ("confession", "custody", "solicitor", "tired", "oppression"),
                    ("confession", "pace s 76", "section 76", "s 76", "oppression", "unreliability", "section 78", "s 78"),
                    "PACE confession route",
                ),
                (
                    ("silent", "silence", "remained silent"),
                    ("silence", "adverse inference", "section 34", "s 34", "cjpoa", "legal advice"),
                    "adverse inference from silence route",
                ),
                (
                    ("friend", "statement", "refuses to attend", "absent", "hearsay", "frightened"),
                    ("hearsay", "criminal justice act 2003", "cja 2003", "absent witness", "fear", "interests of justice"),
                    "hearsay/absent-witness gateway",
                ),
                (
                    ("previous conviction", "bad character", "burglary"),
                    ("bad character", "criminal justice act 2003", "cja 2003", "gateway", "propensity", "section 101", "s 101"),
                    "bad character gateway and prejudice analysis",
                ),
                (
                    ("unfair", "article 6", "exclude", "exclusion", "pace"),
                    ("section 78", "s 78", "article 6", "fair trial", "exclusion", "discretion"),
                    "exclusionary discretion and Article 6 fairness",
                ),
                (
                    ("admissibility", "weight", "likely weight", "ruling"),
                    ("admissible", "excluded", "weight", "warning", "direction", "ruling"),
                    "item-by-item admissibility, weight and ruling",
                ),
            ]
        elif slug == "trusts_law":
            conditional_moves = [
                (
                    ("certainty", "family", "safe", "trust", "objects", "subject matter", "intention"),
                    ("certainty of intention", "certainty of subject matter", "certainty of objects", "knight", "mcphail", "fixed trust", "discretionary trust"),
                    "three certainties and objects analysis",
                ),
                (
                    ("express trust", "resulting trust", "no trust", "beneficially", "beneficial ownership"),
                    ("express trust", "resulting trust", "no trust", "beneficial", "intention to benefit", "fallback"),
                    "express trust/resulting trust/no-trust classification",
                ),
                (
                    ("breach of trust", "mixed", "business account", "mortgage", "shares", "personal expenses"),
                    ("breach of trust", "fiduciary", "misapplied", "trustee", "personal claim", "proprietary claim"),
                    "breach of trust and personal/proprietary remedy separation",
                ),
                (
                    ("tracing", "common law", "equity", "mixed fund", "mixed account", "lowest intermediate"),
                    ("tracing", "equitable tracing", "common law tracing", "mixed fund", "re hallett", "re oatway", "lowest intermediate balance"),
                    "common law/equitable tracing and mixed-fund route",
                ),
                (
                    ("shares", "double", "doubled", "mortgage", "house", "subrogation", "charge"),
                    ("shares", "proportionate", "lien", "mortgage", "charge", "subrogation", "foskett", "boscawen"),
                    "substitute-asset remedies for shares and mortgage repayment",
                ),
                (
                    ("zara", "daughter", "recipient", "gift", "old debt", "volunteer"),
                    ("recipient", "bona fide purchaser", "value", "volunteer", "knowing receipt", "notice", "unconscionable"),
                    "recipient liability and bona-fide-purchaser/value analysis",
                ),
                (
                    ("insolvent", "insolvency", "creditors", "recover as much"),
                    ("insolvency", "secured", "unsecured", "creditor", "proprietary", "estate", "priority"),
                    "insolvency priority consequences",
                ),
            ]
        elif slug == "criminal_law":
            conditional_moves = [
                (
                    ("burglary", "warehouse", "night", "enters", "entered", "trespasser"),
                    ("burglary", "section 9", "s 9", "trespasser", "building", "aggravated burglary"),
                    "burglary/aggravated-burglary route",
                ),
                (
                    ("laptops", "steal", "stolen", "theft", "dishonesty"),
                    ("theft", "dishonesty", "appropriation", "property belonging to another", "permanently deprive", "ivey"),
                    "theft and dishonesty route",
                ),
                (
                    ("robbery", "force", "knife", "stabs", "guard", "takes"),
                    ("robbery", "force", "in order to steal", "at the time of stealing", "section 8", "s 8"),
                    "robbery force/timing route",
                ),
                (
                    ("stab", "stabs", "security guard", "serious injuries", "wounding", "gbh"),
                    ("section 18", "s 18", "section 20", "s 20", "wounding", "gbh", "grievous bodily harm"),
                    "non-fatal offences route",
                ),
                (
                    ("jogee", "secondary liability", "agreed", "agrees", "getaway", "avoid violence", "knife"),
                    ("jogee", "secondary liability", "assist", "encourage", "intent", "foresight", "conditional intent"),
                    "post-Jogee secondary liability route",
                ),
                (
                    ("withdraw", "withdrew", "remained involved", "avoid violence"),
                    ("withdrawal", "withdraw", "unequivocal", "neutralise", "continued participation", "becerra"),
                    "withdrawal/continued participation route",
                ),
                (
                    ("blood", "drive", "drove", "hides", "hid", "garage", "assisting an offender", "handling"),
                    ("assisting an offender", "criminal law act 1967", "handling stolen goods", "section 22", "s 22", "after the event"),
                    "after-the-event liability and handling route",
                ),
                (
                    ("defence", "defences", "panic", "self-defence", "avoid violence"),
                    ("defence", "self-defence", "panic", "duress", "intoxication", "reasonable force", "no substantial defence"),
                    "possible defences route",
                ),
            ]
        elif slug == "tort_law":
            conditional_moves = [
                (
                    ("negligence", "duty", "careless", "accident", "injury", "loss"),
                    ("duty of care", "established category", "caparo", "robinson", "assumption of responsibility", "omission"),
                    "duty gateway/category route",
                ),
                (
                    ("breach", "standard", "reasonable", "professional", "risk"),
                    ("breach", "standard of care", "reasonable person", "bolam", "bolitho", "magnitude of risk"),
                    "breach/standard route",
                ),
                (
                    ("causation", "caused", "but for", "material contribution", "loss of chance"),
                    ("causation", "but for", "material contribution", "scope of duty", "novus actus", "loss of chance"),
                    "causation/scope route",
                ),
                (
                    ("remoteness", "foreseeable", "damage", "loss"),
                    ("remoteness", "foreseeability", "wagon mound", "scope of liability", "kind of damage"),
                    "remoteness/loss type route",
                ),
                (
                    ("contributory", "defence", "volenti", "mitigation"),
                    ("contributory negligence", "volenti", "illegality", "mitigation", "defence"),
                    "defences/mitigation route",
                ),
                (
                    ("damages", "injunction", "remedy", "remedies", "loss"),
                    ("damages", "injunction", "declaration", "quantum", "contributory", "remedy"),
                    "tort remedy and quantum route",
                ),
            ]
        elif slug == "land_law":
            conditional_moves = [
                (
                    ("registered", "unregistered", "title", "purchaser", "buyer"),
                    ("registered land", "unregistered land", "lra 2002", "section 27", "section 29", "priority"),
                    "registered/unregistered and priority route",
                ),
                (
                    ("legal", "equitable", "interest", "proprietary", "right"),
                    ("legal interest", "equitable interest", "formality", "registration", "notice", "restriction"),
                    "interest/formality/protection route",
                ),
                (
                    ("occupation", "overriding", "overreaching", "trustees", "actual occupation"),
                    ("actual occupation", "overriding interest", "overreaching", "schedule 3", "two trustees"),
                    "overriding/overreaching route",
                ),
                (
                    ("co-owner", "co ownership", "family home", "beneficial share", "tolata"),
                    ("constructive trust", "resulting trust", "beneficial shares", "tolata", "stack", "jones v kernott"),
                    "co-ownership acquisition/quantification route",
                ),
                (
                    ("easement", "covenant", "right of way", "parking", "restrictive covenant"),
                    ("easement", "covenant", "benefit", "burden", "re ellenborough", "registration"),
                    "easement/covenant validity and transmission route",
                ),
                (
                    ("remedy", "remedies", "sale", "possession", "injunction", "declaration"),
                    ("declaration", "injunction", "order for sale", "possession", "damages", "priority"),
                    "land remedy/practical order route",
                ),
            ]
        elif slug == "employment_law":
            conditional_moves = [
                (
                    ("employee", "worker", "self-employed", "contractor", "status"),
                    ("employee", "worker", "self-employed", "personal service", "control", "mutuality", "substitution"),
                    "employment status gateway",
                ),
                (
                    ("dismissal", "unfair dismissal", "redundancy", "misconduct"),
                    ("unfair dismissal", "potentially fair reason", "reasonable investigation", "range of reasonable responses", "polkey", "redundancy"),
                    "dismissal fairness route",
                ),
                (
                    ("discrimination", "equal pay", "harassment", "victimisation", "pregnancy", "disability"),
                    ("direct discrimination", "indirect discrimination", "harassment", "victimisation", "burden of proof", "justification", "reasonable adjustments"),
                    "discrimination/equality route",
                ),
                (
                    ("whistleblowing", "protected disclosure", "detriment"),
                    ("protected disclosure", "public interest", "detriment", "automatic unfair dismissal", "causation"),
                    "whistleblowing route",
                ),
                (
                    ("restrictive covenant", "non-compete", "non compete", "non-solicitation", "garden leave"),
                    ("restraint of trade", "legitimate business interest", "reasonableness", "severance", "injunction"),
                    "restrictive covenant route",
                ),
                (
                    ("time limit", "limitation", "acas", "remedy", "compensation", "reinstatement"),
                    ("time limit", "acas", "early conciliation", "compensation", "reinstatement", "re-engagement", "remedy"),
                    "employment time-limit/remedy route",
                ),
            ]
        elif slug == "family_law":
            conditional_moves = [
                (
                    ("child", "children", "arrangements", "contact", "residence", "parental responsibility"),
                    ("welfare", "welfare checklist", "children act 1989", "parental responsibility", "child arrangements", "no order principle"),
                    "children welfare/order route",
                ),
                (
                    ("financial remedy", "divorce", "matrimonial", "needs", "sharing", "compensation"),
                    ("section 25", "matrimonial causes act", "needs", "sharing", "compensation", "clean break", "financial remedy"),
                    "financial remedy section 25 route",
                ),
                (
                    ("domestic abuse", "non-molestation", "occupation order", "harm"),
                    ("non-molestation", "occupation order", "family law act 1996", "significant harm", "balance of harm"),
                    "domestic-abuse protective-order route",
                ),
                (
                    ("nuptial", "prenup", "pre-nuptial", "agreement"),
                    ("nuptial agreement", "radmacher", "needs", "fairness", "autonomy"),
                    "nuptial agreement fairness route",
                ),
                (
                    ("jurisdiction", "habitual residence", "relocation", "abduction"),
                    ("habitual residence", "jurisdiction", "relocation", "hague convention", "welfare"),
                    "jurisdiction/relocation route",
                ),
                (
                    ("remedy", "order", "likely outcome", "advise"),
                    ("order", "undertaking", "financial order", "child arrangements order", "injunction", "likely outcome"),
                    "family remedy/order route",
                ),
            ]
        elif slug == "commercial_law":
            conditional_moves = [
                (
                    ("sale of goods", "goods", "quality", "description", "fitness", "delivery"),
                    ("sale of goods act", "description", "satisfactory quality", "fitness for purpose", "delivery", "rejection"),
                    "sale-of-goods implied-terms route",
                ),
                (
                    ("title", "nemo dat", "owner", "buyer", "seller", "retention of title"),
                    ("nemo dat", "title", "sale of goods act", "voidable title", "retention of title", "priority"),
                    "title/nemo-dat/retention route",
                ),
                (
                    ("agent", "agency", "principal", "authority", "apparent authority"),
                    ("actual authority", "apparent authority", "agency", "principal", "ratification", "warranty of authority"),
                    "agency authority route",
                ),
                (
                    ("secured", "security", "charge", "floating", "fixed", "registration"),
                    ("fixed charge", "floating charge", "registration", "priority", "companies act 2006", "security"),
                    "secured-transactions priority route",
                ),
                (
                    ("carriage", "bill of lading", "carrier", "shipment"),
                    ("bill of lading", "carrier", "hague-visby", "carriage", "incoterms", "risk"),
                    "carriage/risk route",
                ),
                (
                    ("remedy", "damages", "reject", "termination", "price", "specific performance"),
                    ("rejection", "damages", "price", "specific performance", "termination", "mitigation"),
                    "commercial remedy route",
                ),
            ]
        elif slug == "business_law":
            conditional_moves = [
                (
                    ("director", "directors", "duty", "conflict", "company"),
                    ("companies act 2006", "section 171", "section 172", "section 174", "section 175", "conflict", "authorisation"),
                    "directors' duties statutory route",
                ),
                (
                    ("shareholder", "minority", "unfair prejudice", "derivative"),
                    ("unfair prejudice", "section 994", "derivative claim", "permission", "proper claimant", "remedy"),
                    "shareholder remedy route",
                ),
                (
                    ("insolvent", "insolvency", "liquidation", "administration", "creditors"),
                    ("insolvency", "administration", "liquidation", "creditors", "wrongful trading", "preference", "transaction at an undervalue"),
                    "insolvency office-holder route",
                ),
                (
                    ("wrongful trading", "fraudulent trading", "misfeasance", "phoenix"),
                    ("wrongful trading", "fraudulent trading", "misfeasance", "section 214", "section 213", "section 212", "disqualification"),
                    "insolvent-director liability route",
                ),
                (
                    ("partnership", "partner", "llp", "agency"),
                    ("partnership act 1890", "partner", "agency", "fiduciary", "dissolution", "llp"),
                    "partnership/LLP route",
                ),
                (
                    ("remedy", "compensation", "contribution", "account", "order"),
                    ("compensation", "contribution", "account of profits", "injunction", "buy-out", "disqualification", "remedy"),
                    "business-law remedy route",
                ),
            ]
        elif slug == "eu_law":
            conditional_moves = [
                (
                    ("direct effect", "directive", "regulation", "treaty", "member state"),
                    ("direct effect", "vertical", "horizontal", "directive", "regulation", "treaty article", "implementation"),
                    "direct-effect/instrument route",
                ),
                (
                    ("supremacy", "retained", "assimilated", "withdrawal", "brexit"),
                    ("supremacy", "withdrawal act", "retained eu law", "assimilated law", "interpretation", "disapplication"),
                    "post-Brexit status/interpretation route",
                ),
                (
                    ("free movement", "article 34", "article 45", "restriction", "market access"),
                    ("free movement", "restriction", "market access", "justification", "proportionality", "mandatory requirement"),
                    "free-movement restriction/justification route",
                ),
                (
                    ("preliminary reference", "court of justice", "cjeu", "article 267"),
                    ("article 267", "preliminary reference", "court of justice", "acte clair", "acte éclairé"),
                    "preliminary reference route",
                ),
                (
                    ("state liability", "francovich", "damages", "effective remedy"),
                    ("state liability", "francovich", "sufficiently serious breach", "causal link", "remedy"),
                    "EU remedy/state-liability route",
                ),
                (
                    ("proportionality", "justification", "derogation"),
                    ("legitimate aim", "suitability", "necessity", "proportionality", "derogation"),
                    "EU proportionality route",
                ),
            ]
        elif slug == "human_rights_law":
            conditional_moves = [
                (
                    ("human rights", "hra", "public authority", "victim"),
                    ("human rights act 1998", "public authority", "section 6", "victim", "section 7"),
                    "HRA gateway/public-authority route",
                ),
                (
                    ("article", "interference", "engaged", "right"),
                    ("engagement", "interference", "prescribed by law", "legitimate aim", "necessity", "proportionality"),
                    "right engagement/interference route",
                ),
                (
                    ("article 8", "privacy", "family life", "home"),
                    ("article 8", "private life", "family life", "home", "correspondence", "fair balance"),
                    "Article 8 route",
                ),
                (
                    ("article 10", "expression", "speech", "press"),
                    ("article 10", "freedom of expression", "democratic society", "necessity", "proportionality"),
                    "Article 10 route",
                ),
                (
                    ("article 14", "discrimination", "status"),
                    ("article 14", "ambit", "status", "analogous situation", "justification"),
                    "Article 14 route",
                ),
                (
                    ("remedy", "declaration", "damages", "incompatibility"),
                    ("section 3", "section 4", "declaration of incompatibility", "damages", "just satisfaction", "remedy"),
                    "HRA remedy route",
                ),
            ]
        elif slug == "public_international_law":
            conditional_moves = [
                (
                    ("state responsibility", "breach", "attribution", "internationally wrongful"),
                    ("state responsibility", "attribution", "breach", "internationally wrongful act", "articles on state responsibility"),
                    "state responsibility route",
                ),
                (
                    ("use of force", "self-defence", "armed attack", "security council"),
                    ("article 2(4)", "article 51", "use of force", "armed attack", "necessity", "proportionality", "security council"),
                    "use-of-force/self-defence route",
                ),
                (
                    ("immunity", "state immunity", "official", "head of state", "foreign state"),
                    ("state immunity", "immunity ratione personae", "immunity ratione materiae", "state immunity act", "exception"),
                    "immunity route",
                ),
                (
                    ("custom", "customary", "treaty", "vienna convention", "interpretation"),
                    ("customary international law", "state practice", "opinio juris", "vienna convention", "article 31", "article 32"),
                    "sources/treaty interpretation route",
                ),
                (
                    ("icc", "rome statute", "war crime", "genocide", "crimes against humanity"),
                    ("rome statute", "icc", "jurisdiction", "complementarity", "immunity", "war crime"),
                    "international criminal jurisdiction route",
                ),
                (
                    ("remedy", "reparation", "countermeasure", "injunction", "declaration"),
                    ("reparation", "cessation", "assurances", "countermeasures", "declaration", "compensation"),
                    "PIL remedy/consequence route",
                ),
            ]
        elif slug == "tax_law":
            conditional_moves = [
                (
                    ("income tax", "employment income", "trading income", "benefit", "deduction"),
                    ("income tax", "employment income", "trading income", "deductible", "wholly and exclusively", "benefit in kind"),
                    "income-tax classification/deduction route",
                ),
                (
                    ("capital gains", "cgt", "disposal", "asset", "gain"),
                    ("capital gains tax", "disposal", "chargeable gain", "base cost", "relief", "principal private residence"),
                    "CGT disposal/gain route",
                ),
                (
                    ("vat", "supply", "taxable", "exempt", "input tax", "output tax"),
                    ("vat", "taxable supply", "exempt", "input tax", "output tax", "registration"),
                    "VAT supply/registration route",
                ),
                (
                    ("avoidance", "gaar", "ramsay", "scheme", "tax advantage"),
                    ("avoidance", "gaar", "ramsay", "tax advantage", "abusive", "purposive construction"),
                    "avoidance/GAAR/Ramsay route",
                ),
                (
                    ("penalty", "enquiry", "assessment", "appeal", "hmrc"),
                    ("assessment", "enquiry", "penalty", "reasonable excuse", "appeal", "hmrc"),
                    "tax administration/enforcement route",
                ),
                (
                    ("relief", "loss", "remedy", "liability", "pay"),
                    ("relief", "allowance", "loss relief", "tax liability", "interest", "penalty", "appeal"),
                    "tax liability/remedy route",
                ),
            ]
        else:
            continue

        for prompt_terms, answer_terms, label in conditional_moves:
            if _has_any_text(scope_low, prompt_terms) and not _answer_has_any_term(answer_low, answer_terms):
                missing.append(label)
        if missing:
            issues.append(
                f"Subject-specific route gate under-covered for {slug}: "
                + "; ".join(missing[:6])
            )

    # Deduplicate where a multi-question prompt and its unit text both trigger.
    deduped: List[str] = []
    seen = set()
    for issue in issues:
        key = issue.lower()
...
def _complete_answer_forbidden_source_list_issues(answer_text: str, prompt_text: str) -> List[str]:
    """
    Complete-answer mode integrates authorities inline. A separate research log
    is only acceptable when the user expressly asks for one.
    """
    ...
    prompt_low = (prompt_text or "").lower()
    if re.search(
        r"\b(?:bibliography|reference list|references list|works cited|table of cases|table of authorities|source list|sources list|list the sources|include sources|provide sources|provide links|web links)\b",
        prompt_low,
    ):
        return []
    forbidden = re.search(
        r"(?im)^\s*(?:online verification used|online search used|sources checked|source[s]? used|primary sources checked|research sources|rag sources|rag context|retrieval log)\b",
        txt,
    )
    if not forbidden:
        return []
    return [
        "Forbidden detached source/research log in exam-style complete answer; integrate authorities inline unless the user expressly requested a source list."
    ]


def _complete_answer_final_tail(answer_text: str, *, max_words: int = 360) -> str:
    cleaned = re.sub(r"(?im)^\s*\(End of Answer\)\s*$", "", answer_text or "").strip()
    if not cleaned:
        return ""
    final_heading = list(
        re.finditer(
            r"(?im)^\s*(?:part\s+[ivxlcdm0-9]+\s*:\s*)?(?:final\s+)?(?:conclusion|outcome|advice|remedies?\s*/\s*liability|remedies?|liability)\b.*$",
            cleaned,
        )
    )
    if final_heading:
        tail = cleaned[final_heading[-1].start():]
    else:
...
def _subject_specific_final_outcome_issues(answer_text: str, prompt_text: str) -> List[str]:
    """
    A first-class problem answer must end with practical advice, not merely
    doctrinal discussion. These checks are intentionally broad and party/remedy
    focused so they catch missing final rankings without enforcing a template.
    """
    if not (answer_text or "").strip():
        return []
    tail = _complete_answer_final_tail(answer_text)
    if not tail:
        return []
    answer_low = (answer_text or "").lower()
    issues: List[str] = []
    for scope in _subject_route_prompt_scopes(prompt_text):
        ...
        if slug == "trusts_law" and any(k in scope_low for k in ["shares", "mortgage", "zara", "daughter", "insolvent"]):
            needed_hits = sum(1 for k in ("shares", "mortgage", "zara", "daughter", "insolvency", "proprietary", "unsecured") if k in tail)
            if needed_hits < 3:
                issues.append("Subject-specific final outcome weak for trusts_law: final advice must rank asset-by-asset proprietary recovery, recipient claims and insolvency priority.")
        elif slug == "criminal_law" and any(k in scope_low for k in ["liam", "noah", "defendant", "advise"]):
            party_hits = sum(1 for k in ("liam", "noah", "defendant", "accused") if k in tail)
            liability_hit = any(k in tail for k in ("guilty", "liable", "not liable", "conviction", "offence", "charge"))
            if party_hits < 2 or not liability_hit:
                issues.append("Subject-specific final outcome weak for criminal_law: final advice must rank liability defendant-by-defendant and offence-by-offence.")
        elif slug == "public_law" and any(k in scope_low for k in ["judicial review", "quashing", "declaration", "interim", "remedy", "council"]):
            if not any(k in tail for k in ("quashing", "declaration", "interim", "mandatory order", "prohibiting", "remittal", "remedy")):
                issues.append("Subject-specific final outcome weak for public_law: final advice must state the likely JR remedy and practical effect.")
        elif slug == "contract_law" and any(k in scope_low for k in ["damages", "rescission", "breach", "misrepresentation", "exclusion", "remedy"]):
            if not any(k in tail for k in ("damages", "rescission", "set-off", "termination", "price reduction", "loss of chance", "recover")):
                issues.append("Subject-specific final outcome weak for contract_law: final advice must state likely remedies and limits on recovery.")
        elif slug == "evidence_law" and any(k in scope_low for k in ["admissibility", "weight", "evidence", "prosecution"]):
            if not any(k in tail for k in ("admissible", "excluded", "weight", "warning", "prosecution case", "fair trial")):
                issues.append("Subject-specific final outcome weak for evidence_law: final advice must state admissibility/weight and overall case strength.")
        elif slug == "tort_law" and any(k in scope_low for k in ["injury", "loss", "damage", "negligence", "nuisance", "defamation"]):
            if not any(k in tail for k in ("liable", "liability", "damages", "injunction", "contributory", "recover", "remedy")):
                issues.append("Subject-specific final outcome weak for tort_law: final advice must state likely liability, remedy and practical limit.")
        elif slug == "land_law" and any(k in scope_low for k in ["purchaser", "priority", "occupation", "easement", "covenant", "co-owner", "possession"]):
            if not any(k in tail for k in ("priority", "binding", "overriding", "overreached", "declaration", "injunction", "order for sale", "possession")):
                issues.append("Subject-specific final outcome weak for land_law: final advice must state whether the right binds and what order/remedy follows.")
        elif slug == "employment_law" and any(k in scope_low for k in ["dismissal", "worker", "employee", "discrimination", "covenant", "claim"]):
            if not any(k in tail for k in ("claim", "compensation", "reinstatement", "injunction", "time limit", "liable", "remedy")):
                issues.append("Subject-specific final outcome weak for employment_law: final advice must state claim strength, time/remedy and employer exposure.")
        elif slug == "family_law" and any(k in scope_low for k in ["child", "financial remedy", "divorce", "order", "welfare"]):
            if not any(k in tail for k in ("order", "welfare", "financial remedy", "clean break", "injunction", "likely outcome")):
                issues.append("Subject-specific final outcome weak for family_law: final advice must state the likely order and welfare/financial outcome.")
        elif slug == "commercial_law" and any(k in scope_low for k in ["goods", "title", "agent", "security", "carrier", "remedy"]):
            if not any(k in tail for k in ("reject", "damages", "price", "title", "priority", "principal", "termination", "remedy")):
                issues.append("Subject-specific final outcome weak for commercial_law: final advice must state commercial remedies and allocation of risk/title/liability.")
        elif slug == "business_law" and any(k in scope_low for k in ["director", "company", "shareholder", "insolvency", "partner"]):
            if not any(k in tail for k in ("compensation", "contribution", "buy-out", "disqualification", "liquidator", "administrator", "remedy", "liable")):
                issues.append("Subject-specific final outcome weak for business_law: final advice must state company/insolvency remedy and responsible party exposure.")
        elif slug == "eu_law" and any(k in scope_low for k in ["directive", "free movement", "direct effect", "eu", "member state"]):
            if not any(k in tail for k in ("direct effect", "disapply", "state liability", "damages", "proportionality", "reference", "remedy")):
                issues.append("Subject-specific final outcome weak for eu_law: final advice must state enforceability, justification and remedy.")
        elif slug == "human_rights_law" and any(k in scope_low for k in ["article", "human rights", "hra", "proportionality"]):
            if not any(k in tail for k in ("breach", "proportionate", "declaration", "damages", "section 3", "section 4", "remedy")):
                issues.append("Subject-specific final outcome weak for human_rights_law: final advice must state breach/proportionality and HRA remedy.")
        elif slug == "public_international_law" and any(k in scope_low for k in ["state", "treaty", "immunity", "force", "icc", "international"]):
            if not any(k in tail for k in ("responsibility", "immunity", "jurisdiction", "reparation", "cessation", "countermeasure", "lawful", "unlawful")):
                issues.append("Subject-specific final outcome weak for public_international_law: final advice must state international-law consequence and remedy.")
        elif slug == "tax_law" and any(k in scope_low for k in ["tax", "hmrc", "vat", "income", "capital gains", "penalty"]):
            if not any(k in tail for k in ("tax liability", "assessment", "relief", "penalty", "appeal", "payable", "hmrc")):
                issues.append("Subject-specific final outcome weak for tax_law: final advice must state liability, relief/penalty and appeal/remedy.")
...
def _complete_answer_allowed_authorities_for_trace(
    prompt_text: str,
    rag_context: Optional[str],
) -> List[str]:
    source_bits = [prompt_text or ""]
    if rag_context and not str(rag_context).startswith("[RAG ERROR]"):
        source_bits.append(str(rag_context))
    if callable(_subject_guide_excerpt_for_query):
        try:
            guide = _subject_guide_excerpt_for_query(prompt_text or "", max_lines=120)
        except Exception:
            guide = ""
        if guide:
            source_bits.append(guide)
    combined = "\n\n".join(bit for bit in source_bits if str(bit or "").strip())
    try:
        return get_allowed_authorities_from_rag(combined, limit=260)
    except Exception:
        return []


def _complete_answer_citation_trace_issues(
    answer_text: str,
    prompt_text: str,
    rag_context: Optional[str],
) -> List[str]:
    """
    Non-destructive citation allow-list validation.

    The final answer should cite only authorities that can be traced to the
    retrieved RAG/search context, the question itself, or the active subject
    guide. This checker reports problems; it does not strip the answer.
    """
    if not rag_context or str(rag_context).startswith("[RAG]") or str(rag_context).startswith("[RAG ERROR]"):
        return []
    allowed = _complete_answer_allowed_authorities_for_trace(prompt_text, rag_context)
    if not allowed:
        return []
    try:
        _sanitized, removed = sanitize_output_against_allowlist(
            answer_text or "",
            allowed,
            rag_context_len=len(str(rag_context or "")),
            strict=True,
        )
    except Exception:
        return []
    if not removed:
        return []
    preview = "; ".join(str(item)[:120] for item in removed[:5])
    return [
        "Citation trace verification failed: cited authority is not traceable to retrieved RAG/search context, prompt authorities, or matched subject guide. "
        f"Examples: {preview}"
    ]
...
def _strict_complete_answer_issues(
    ...
    *,
    ...
    issues.extend(_topic_profile_guide_contract_issues(answer_text, prompt_text))
    issues.extend(_law_medicine_surgical_consent_methodology_issues(answer_text, prompt_text))
    issues.extend(_law_medicine_mixed_medical_decision_route_issues(answer_text, prompt_text))
    issues.extend(_subject_specific_problem_route_issues(answer_text, prompt_text))
    issues.extend(_complete_answer_forbidden_source_list_issues(answer_text, prompt_text))
    issues.extend(_subject_specific_final_outcome_issues(answer_text, prompt_text))
    issues.extend(_complete_answer_citation_trace_issues(answer_text, prompt_text, rag_context))
    ...
    if word_window:
        ...
        actual_words = _count_answer_words_for_window(answer_text or "")
...
def _severe_complete_answer_issues(issues: List[str]) -> List[str]:
    """
    Return the subset of complete-answer verification issues that indicate the
    answer is structurally unsafe to show as final output.

    Word-count and citation-density issues can be improved later, but an answer
    with no conclusion or an abrupt/cut-off ending is materially misleading in
    chat/API delivery. This guard is deliberately used after word-cap trimming
    because trimming can otherwise remove the final conclusion and then append
    an "(End of Answer)" marker to an incomplete answer.
    """
    severe_patterns = (
        "ends abruptly",
        ...
        "workspace link",
        "argumentative sentence-support verification failed",
        "citation density is too thin",
        "no visible inline oscola citation support",
        "authority anchors without immediate oscola",
        "case references are not followed by immediate oscola",
        "malformed inline oscola",
        "malformed citation",
        "incomplete case citation",
        "citation trace verification failed",
        "file-name citations",
        "database-label citations",
        "placeholder numeric citation",
        "final planned question-part is incomplete",
        "empty structural headings",
        "part heading with no substantive content",
        "bare lettered subheading",
        "final analytical part is left open",
        "topic guide contract under-covered",
        "law and medicine surgical-consent methodology",
        "law and medicine mixed medical-decision methodology",
        "subject-specific route gate under-covered",
        "subject-specific final outcome weak",
        "forbidden detached source/research log",
    )
    severe: List[str] = []
    for issue in issues or []:
        low = (issue or "").lower()
        if any(pattern in low for pattern in severe_patterns):
            severe.append(issue)
    return severe


def _complete_answer_issue_score(issues: List[str]) -> int:
    """
    Weighted score for choosing the best retry candidate.

    The previous selection used raw issue count, which let a response with no
    usable inline citations survive when a retry had fewer severe legal-quality
    defects but the same number of minor issues. Missing OSCOLA support, source
    leakage, and structural incompleteness are now treated as dominant failures.
    """
    score = 0
    for issue in issues or []:
        low = (issue or "").lower()
        ...
        if any(
            ...
            weight = 80
        elif "law and medicine surgical-consent methodology" in low:
            weight = 75
        elif "law and medicine mixed medical-decision methodology" in low:
            weight = 75
        elif "topic guide contract under-covered" in low:
            weight = 70
        elif "subject-specific route gate under-covered" in low:
            weight = 70
        elif "subject-specific final outcome weak" in low:
            weight = 65
        elif "multi-question" in low or "question 1" in low or "question 2" in low:
            weight = 45
        elif "strict complete-answer word window" in low or "word window" in low:
            weight = 8
        elif "prompt-map asks appear under-covered" in low:
            weight = 25
        score += weight
    return score


def _split_answer_question_blocks(answer_text: str) -> List[Dict[str, Any]]:
    """
    Split a finished answer into visible Question N blocks.

    Multi-question legal answers are allowed to restart Part numbering inside
    each block, so validators need block-level structure rather than one global
    Part sequence.
    """
    body = re.sub(r"\(End of Answer\)\s*$", "", answer_text or "", flags=re.IGNORECASE).strip()
    if not body:
        return []
    matches = list(re.finditer(r"(?im)^\s*Question\s+(\d+)\s*:\s*.+?$", body))
    ...
    for idx, match in enumerate(matches):
        ...
        blocks.append({
            "number": number,
            "heading": (match.group(0) or "").strip(),
            "text": body[start:end].strip(),
        })
    return blocks


def _prompt_units_for_complete_answer(prompt_text: str) -> List[Dict[str, Any]]:
    if callable(_extract_units_with_text):
        try:
            units = _extract_units_with_text(prompt_text or "")
            if units:
                return [
                    {
                        "label": str(u.get("label") or f"Question {idx}"),
                        "text": str(u.get("text") or ""),
                        "kind": str(u.get("kind") or "general").strip().lower() or "general",
                        "question_index": int(u.get("question_index", idx) or idx),
                    }
...
def _question_kind_for_unit(unit: Optional[Dict[str, Any]], prompt_text: str) -> str:
    unit_text = ""
    unit_kind = ""
    if unit:
        unit_kind = str(unit.get("kind") or "").strip().lower()
        unit_text = str(unit.get("text") or unit.get("label") or "")
    low = f"{unit_kind}\n{unit_text}\n{prompt_text or ''}".lower()
    if unit_kind in {"problem", "pb", "problem question", "pq"}:
        return "problem"
    if unit_kind in {"essay", "essay question"}:
        return "essay"
    if "problem question" in low or re.search(r"\badvise\b", low):
        return "problem"
    if "essay question" in low or "critically" in low or re.search(r"\bdiscuss\b|\bevaluate\b", low):
        return "essay"
    return "general"


def _part_numbers_in_block(block_text: str) -> List[int]:
    nums: List[int] = []
    for match in re.finditer(r"(?im)^\s*Part\s+([IVXLCDM]+|\d+)\s*:\s*", block_text or ""):
        token = (match.group(1) or "").strip()
        try:
            nums.append(int(token) if token.isdigit() else _roman_to_int(token))
        except Exception:
            continue
    return nums


def _multi_question_complete_answer_issues(answer_text: str, prompt_text: str) -> List[str]:
    """
    Enforce the contract the user cares about most: numbered questions with
    repeated word targets must be answered as separate full answers, not one
    compressed combined note.
    """
    issues: List[str] = []
    targets = _extract_word_targets(prompt_text or "")
    units = _prompt_units_for_complete_answer(prompt_text or "")
    expected_count = max(
        len(targets) if len(targets) >= 2 else 0,
        len(units) if len(units) >= 2 else 0,
    )
    if expected_count < 2:
        return issues

    blocks = _split_answer_question_blocks(answer_text or "")
    if len(blocks) < expected_count:
        issues.append(
            f"Multi-question answer must use separate Question headings for all {expected_count} questions; found {len(blocks)}."
        )
        return issues
    ...
    for idx in range(expected_count):
        ...
        question_label = f"Question {idx + 1}"
        part_nums = _part_numbers_in_block(block_text)
        if not part_nums:
            issues.append(f"{question_label} lacks Part-numbered structure.")
        elif part_nums[0] != 1:
            issues.append(f"{question_label} must restart its own Part numbering at Part I.")
        else:
            for pos in range(1, len(part_nums)):
                if part_nums[pos] <= part_nums[pos - 1]:
                    issues.append(f"{question_label} repeats or regresses Part numbering within its own block.")
                    break

        target = int(targets[idx]) if idx < len(targets) else 0
        if target > 0:
            words = _count_words(block_text)
            floor = max(350, complete_word_count_floor(target))
            if words < floor:
                issues.append(
                    f"{question_label} under-delivers on its own requested word target ({words} words; expected at least {floor})."
                )
            if words > target:
                issues.append(
                    f"{question_label} exceeds its own requested word target ({words} words; cap is {target})."
                )

        unit = units[idx] if idx < len(units) else None
        kind = _question_kind_for_unit(unit, prompt_text)
        titles = [
            (m.group(2) or "").strip().lower()
            for m in re.finditer(r"(?im)^\s*Part\s+([IVXLCDM]+|\d+)\s*:\s*(.+?)\s*$", block_text)
        ]
        if kind == "problem":
            if not any(_is_problem_remedies_liability_title(t) for t in titles):
                issues.append(f"{question_label} problem answer is missing its own Remedies / Liability Part.")
            if not any(_is_problem_final_conclusion_title(t) for t in titles):
                issues.append(f"{question_label} problem answer is missing its own Final Conclusion Part.")
        elif kind == "essay":
            if not any("conclusion" in t for t in titles):
                issues.append(f"{question_label} essay answer is missing its own Part-numbered Conclusion.")

    early_end_markers = list(re.finditer(r"\(End of Answer\)", answer_text or "", flags=re.IGNORECASE))
    if len(early_end_markers) > 1:
        issues.append("Multi-question answer must use exactly one '(End of Answer)' marker after the final question only.")
    elif early_end_markers:
        marker = early_end_markers[0]
        tail = (answer_text or "")[marker.end():]
        if re.search(r"(?im)^\s*Question\s+\d+\s*:", tail):
            issues.append("Multi-question answer places '(End of Answer)' before a later question.")
    return issues
...
def _direct_complete_answer_structure_issues(
    ...
    prompt_targets = _extract_word_targets(prompt_text or "")
    prompt_units = _prompt_units_for_complete_answer(prompt_text or "")
    multi_question_issues = _multi_question_complete_answer_issues(answer_text, prompt_text)
    expected_multi_question_count = max(
        len(prompt_targets) if len(prompt_targets) >= 2 else 0,
        len(prompt_units) if len(prompt_units) >= 2 else 0,
    )
    if expected_multi_question_count >= 2:
        # Avoid running a single-answer scaffold over a deliberately restarted
        # multi-question answer; the block-level contract above is more precise.
        return multi_question_issues
    ...
    *,
    ...
    return "\n".join(
        [
            ...
            "Treat this as a top-band Codex supervisor rewrite across every law subject, regardless of whether the user used words like first-class, xhigh, or supervisor.",
            "Use indexed RAG and the shared backend code-guide instructions already active for this request.",
            "Before rewriting, redo the evidence-first pass: read the retrieved RAG/search/source-guide material, build an internal issue-source map for every prompt limb, then draft from that map rather than from memory. If online search is used, it must discover and validate materially relevant current/updated sources across the prompt, not just one supporting link.",
            "Apply the detected topic/profile guide across the whole answer: expected keywords, issue-bank moves, must-avoid rules, counterarguments, remedy focus, and matched subject-guide anchors are mandatory quality controls where they fit the prompt.",
            "For Law and Medicine surgical-consent problems, keep right/left procedures separate: informed-consent negligence for disclosed/undisclosed risks and alternatives, battery/scope of consent for any additional body-part procedure, narrow necessity, causation, autonomy/loss-of-chance limits, and remedies/loss.",
            "For mixed Law and Medicine hospital problems, keep each statutory route separate: adult refusal/capacity under the MCA and Re C/Re B/Re T; child treatment refusal under Gillick/FLRA/parental responsibility/welfare and court override; living child organ donation under HTA 2004/Code F plus court scrutiny; and ventilator/advance-decision analysis under MCA ss 24-26, validity, applicability, best interests, and urgent court/clinical next steps.",
            "Ensure every argumentative legal-proposition sentence is individually supported by a terminal inline authority citation in the pattern sentence (citation), or rewrite it more cautiously so it does not overstate support.",
            "Do NOT solve citation risk by deleting citations while leaving legal propositions unsupported. Use retrieved RAG, backend-search, or explicit project code-guide authorities where available; if no exact authority is available, narrow or omit the proposition.",
            "For default OSCOLA mode, most substantive legal-proposition sentences must end with a full inline OSCOLA citation in parentheses.",
            ...
            "If the word limit is tight, compress earlier explanation but preserve the final conclusion, final outcome/remedy, and a complete terminal sentence.",
            window_line,
...
def send_complete_answer_with_docs(
    ...
    - every complete-answer output runs the strict supervisor/verification route by default.
    """
    ...
    def _finalize_complete_answer_return(
        ...
        final_text = _maybe_reinforce_law_medicine_statutory_routes(final_text, message)
    ...
    if is_interactive_codex_supervisor_handoff(response_text):
        _cleanup_complete_answer_task_artifacts(cleanup_paths, protected_paths=[])
        return (response_text, response_meta), rag_context

    is_complete_answer_request = _looks_like_backend_complete_answer_request(message, documents)
    if not is_complete_answer_request:
        return _finalize_complete_answer_return(response_text, response_meta, rag_context)

    # Complete legal answers must always pass the backend supervisor gate.
    # Callers may disable this only by using a dedicated non-complete-answer
    # route such as SQE question generation/marking or the DOCX amend workflow;
    # a full legal essay/problem answer cannot silently fall back to the normal
    # provider transport path.
    strict_complete_answer_verification = True
    ...
    best_issues = issues
    best_score = _complete_answer_issue_score(best_issues)
    word_window = _resolve_complete_answer_word_window(
        message,
        history,
        enforce_long_response_split=enforce_split,
    ...
    for _attempt in range(BACKEND_COMPLETE_ANSWER_REGEN_MAX_ATTEMPTS):
        ...
        retry_score = _complete_answer_issue_score(retry_issues)
        if retry_score < best_score or (
            retry_score == best_score and len(retry_issues) < len(best_issues)
        ):
    ...
    if word_window:
        untrimmed_best_text = best_text
        untrimmed_best_meta = best_meta
        trimmed_text = _truncate_to_word_cap(best_text, int(word_window[1]), int(word_window[0]))
        trimmed_issues = _strict_complete_answer_issues(
            trimmed_text,
            message,
            history,
            enforce_long_response_split=enforce_split,
        ...
        if _severe_complete_answer_issues(trimmed_issues) and not _severe_complete_answer_issues(best_issues):
            # Prefer a complete, slightly over-length answer to an artificial
            # cap-trimmed answer that loses the conclusion or ends mid-analysis.
            best_text = untrimmed_best_text
            best_meta = untrimmed_best_meta
        else:
...
def _essay_quality_issues(
    ...
    if is_problem_mode:
        problem_question_blocks = _split_answer_question_blocks(txt)
        if len(problem_question_blocks) >= 2:
            for block in problem_question_blocks:
                for issue in _problem_structure_issues(str(block.get("text") or "")):
                    issues.append(f"Question {block.get('number')} {issue}")
        else:
    ...
    for m in re.finditer(r"(?im)^\s*Part\s+([IVXLC]+)\s*:\s*", txt):
        try:
            ...
            continue
    multi_question_for_parts = len(_split_answer_question_blocks(txt)) >= 2
    if multi_question_for_parts:
        for block in _split_answer_question_blocks(txt):
            block_parts = _part_numbers_in_block(str(block.get("text") or ""))
            if not block_parts:
                continue
            if block_parts[0] != 1:
                issues.append(f"Question {block.get('number')} must restart Part numbering at Part I.")
                break
            for i in range(1, len(block_parts)):
                if block_parts[i] <= block_parts[i - 1]:
                    issues.append(f"Question {block.get('number')} Part numbering regresses or repeats inside that question.")
                    break
    elif len(parts) >= 2:
