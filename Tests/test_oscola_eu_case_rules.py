...
assert "Body-italics strict-scope rule (mandatory)" in guide_text
...
assert not any(run.xpath("./w:rPr/w:i|./w:rPr/w:iCs", namespaces=NS) for run in first_para_runs)
...
short_case_root = etree.fromstring(
    f"""<w:document xmlns:w="{WORD_NS}">
  <w:body>
    <w:p><w:r><w:t>A competent child may consent (Gillick), but ordinary words must remain roman.</w:t></w:r></w:p>
    <w:p><w:r><w:t>Capacity is preserved (Re C). However, the facts may differ.</w:t></w:r></w:p>
    <w:p><w:r><w:t>The Human Tissue Authority (HTA) and NHS Blood and Transplant (NHSBT) remain roman.</w:t></w:r></w:p>
    <w:p><w:r><w:t>X Primary Care Trust v XB and YB [2012] EWHC 1390 (Fam) remains relevant.</w:t></w:r></w:p>
  </w:body>
</w:document>"""
)
assert _normalize_body_italics(short_case_root, None) >= 1
short_case_runs = short_case_root.xpath("/w:document/w:body/w:p[1]/w:r", namespaces=NS)
assert any(
    "Gillick" in "".join(run.xpath("./w:t/text()", namespaces=NS))
    and run.xpath("./w:rPr/w:i|./w:rPr/w:iCs", namespaces=NS)
    for run in short_case_runs
)
assert not any(
    "ordinary words" in "".join(run.xpath("./w:t/text()", namespaces=NS))
    and run.xpath("./w:rPr/w:i|./w:rPr/w:iCs", namespaces=NS)
    for run in short_case_runs
)
sentence_case_runs = short_case_root.xpath("/w:document/w:body/w:p[2]/w:r", namespaces=NS)
assert any(
    "Re C" in "".join(run.xpath("./w:t/text()", namespaces=NS))
    and run.xpath("./w:rPr/w:i|./w:rPr/w:iCs", namespaces=NS)
    for run in sentence_case_runs
)
assert not any(
    "However" in "".join(run.xpath("./w:t/text()", namespaces=NS))
    and run.xpath("./w:rPr/w:i|./w:rPr/w:iCs", namespaces=NS)
    for run in sentence_case_runs
)
acronym_runs = short_case_root.xpath("/w:document/w:body/w:p[3]/w:r", namespaces=NS)
assert not any(run.xpath("./w:rPr/w:i|./w:rPr/w:iCs", namespaces=NS) for run in acronym_runs)
citation_runs = short_case_root.xpath("/w:document/w:body/w:p[4]/w:r", namespaces=NS)
assert any(
    "X Primary Care Trust v XB and YB" in "".join(run.xpath("./w:t/text()", namespaces=NS))
    and run.xpath("./w:rPr/w:i|./w:rPr/w:iCs", namespaces=NS)
    for run in citation_runs
)
assert not any(
    "Fam" in "".join(run.xpath("./w:t/text()", namespaces=NS))
    and run.xpath("./w:rPr/w:i|./w:rPr/w:iCs", namespaces=NS)
    for run in citation_runs
)
