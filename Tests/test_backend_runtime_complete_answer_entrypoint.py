...
assert "top-band Codex supervisor rewrite across every law subject" in captured_calls[1]["message"]
...
forced_supervisor_calls = []
forced_supervisor_responses = [
    (("LOW QUALITY DRAFT", []), "Authority: Montgomery v Lanarkshire Health Board [2015] UKSC 11."),
    (("SUPERVISED FINAL", []), "Authority: Montgomery v Lanarkshire Health Board [2015] UKSC 11."),
]


def fake_provider_for_forced_supervisor(
    api_key,
    message,
    documents,
    project_id,
    ...
    forced_supervisor_calls.append(message)
    idx = min(len(forced_supervisor_calls) - 1, len(forced_supervisor_responses) - 1)
    return forced_supervisor_responses[idx]


def fake_forced_supervisor_issues(
    answer_text,
    prompt_text,
    messages,
    *,
    ...
    if answer_text == "LOW QUALITY DRAFT":
        return ["Answer is below the strict complete-answer word window (100 words; need at least 1800)."]
    return []
...
original_issue_checker = runtime._strict_complete_answer_issues
try:
    runtime._provider_send_message_with_docs = fake_provider_for_forced_supervisor
    runtime._strict_complete_answer_issues = fake_forced_supervisor_issues
    (forced_text, _forced_meta), _forced_rag = runtime.send_complete_answer_with_docs(
        api_key="",
        message="""Law and Medicine — Problem Question

Word target: about 2,000 words

Advise the patient and hospital on informed consent after Montgomery.""",
        documents=[],
        project_id="forced-supervisor",
        history=[],
        stream=False,
...
assert forced_text == "SUPERVISED FINAL"
assert len(forced_supervisor_calls) == 2
assert "[BACKEND STRICT COMPLETE-ANSWER REWRITE]" in forced_supervisor_calls[1]
assert "top-band Codex supervisor rewrite across every law subject" in forced_supervisor_calls[1]
...
original_provider_codex_supports_option = provider_service._codex_exec_supports_option
...
original_codex_assume_env = os.environ.get("LEGAL_AI_CODEX_ASSUME_NETWORK_AVAILABLE")
try:
    ...
    provider_service._codex_exec_supports_option = lambda cli, option: option == "--search"
    ...
    provider_service._codex_exec_supports_option = original_provider_codex_supports_option
    ...
    if original_codex_assume_env is None:
        os.environ.pop("LEGAL_AI_CODEX_ASSUME_NETWORK_AVAILABLE", None)
    else:
        os.environ["LEGAL_AI_CODEX_ASSUME_NETWORK_AVAILABLE"] = original_codex_assume_env
