...
- **CANONICAL FINAL OUTPUT OVERWRITE RULE.** Never overwrite the user's original source DOCX, but do overwrite the canonical Desktop final amended DOCX for that source on later amend runs rather than creating `_v2`, `_v3`, and so on.
...
- **Single canonical Desktop output is mandatory.** After a successful amend run, keep one final amended DOCX in Desktop root at the canonical filename for that source. If the canonical filename already exists, overwrite that final system output instead of creating `_v2`, `_v3`, and so on.
...
- **Single-run output policy.** For each amend run, generate one final marked DOCX and present only that final file path. Reuse the same canonical final filename for that source rather than accumulating versioned siblings.
...
7. Output location and naming:
   ...
   - Use the canonical final filename per source (recommended suffix: `_amended_marked_final.docx`) as the output path. Reuse that same final filename on later amend runs instead of creating `_v2`, `_v3`, and so on.
