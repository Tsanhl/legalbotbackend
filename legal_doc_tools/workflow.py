...
AMEND_SUPERVISOR_MAX_ATTEMPTS = 2
...
def _build_structured_amend_retry_prompt(
    *,
    base_prompt: str,
    failure: str,
    ...
    failure_excerpt = re.sub(r"\s+", " ", str(failure or "")).strip()[:5000]
    response_excerpt = str(previous_response or "").strip()[:5000]
    return "\n\n".join(
        part
        for part in [
            base_prompt,
            "[AMEND SUPERVISOR RETRY - RETURN JSON ONLY]",
            (
                f"The previous amendment plan failed strict amend validation on attempt {attempt - 1}. "
                "Regenerate the full JSON plan from scratch while preserving the original DOCX structure and all controlling instructions."
            ),
            f"Failure to fix:\n{failure_excerpt or 'Unknown validation failure.'}",
            (
                "Previous response excerpt for diagnosis only; do not copy broken JSON blindly:\n"
                + response_excerpt
                if response_excerpt
                else ""
            ),
            "Mandatory repair rules: return one valid JSON object only; include every required report artifact; keep all original footnote IDs and bibliography/reference handling safe; make at least one detectable amendment unless the user expressly asked for review-only output; and ensure the delivery gate would report zero unverified items.",
        ]
...
- use indexed materials plus online verification and search-backed supplementation where helpful; online verification must cover materially relevant current/updated sources for the prompt rather than a token citation check; use any backend online-search context supplied by the route, but never invent authorities.
...
def run_uploaded_legal_doc_amend_workflow(
    *,
    ...
    try:
        ...
        last_failure = ""
        last_response_text = ""
        for attempt in range(1, AMEND_SUPERVISOR_MAX_ATTEMPTS + 1):
            active_prompt = prompt
            if attempt > 1:
                active_prompt = _build_structured_amend_retry_prompt(
                    base_prompt=prompt,
            ...
            try:
                ...
                if is_interactive_codex_supervisor_handoff(response_text):
                    raise RuntimeError(
                        "Interactive Codex handoff required for DOCX amendment generation. "
                        "The backend prepared RAG/code-guide/supervisor context but cannot produce the JSON amend plan in this runtime.\n\n"
                        f"{response_text}"
                    )
                ...
                changed_paragraphs = sum(
                    1
                    for old_text, new_text in zip(snapshot.paragraph_texts, amended_paragraphs)
                    if old_text != new_text
                )
                ...
                if "Interactive Codex handoff required" in last_failure or is_interactive_codex_supervisor_handoff(last_response_text):
                    raise
                if attempt >= AMEND_SUPERVISOR_MAX_ATTEMPTS:
                    raise
    finally:
