 ...
 * Run with: npx ts-node scripts/indexLawResources.ts
 */

import * as fs from 'fs';
import * as path from 'path';

interface LawResourceEntry {
  id: string;
  name: string;
  path: string;
  category: string;
...
function walkDir(dir: string, category: string = '', subcategory: string = ''): LawResourceEntry[] {
  const entries: LawResourceEntry[] = [];
