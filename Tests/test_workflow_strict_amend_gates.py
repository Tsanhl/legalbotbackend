...
with tempfile.TemporaryDirectory() as tmp_dir:
    ...
    def retrying_send(*args, **kwargs):
        calls.append(args[1])
        if len(calls) == 1:
            return ((_strict_response(include_reports=False), None), None)
        return ((_strict_response(), None), None)

    workflow.DESKTOP_ROOT = desktop
    refine.DESKTOP_ROOT = desktop
    amend_docx.DESKTOP_ROOT = desktop
    delivery_gates.DESKTOP_ROOT = desktop
    workflow.send_message_with_docs = retrying_send
    try:
        result = workflow.run_local_legal_doc_amend_workflow(
            api_key="test-key",
            source_path=source,
        ...
        assert result.output_path.exists()
        assert len(calls) == 2
        assert "[AMEND SUPERVISOR RETRY - RETURN JSON ONLY]" in calls[1]
        assert "authority_verification_report" in calls[1]
    finally:
        workflow.DESKTOP_ROOT = original_workflow_desktop
        refine.DESKTOP_ROOT = original_refine_desktop
        amend_docx.DESKTOP_ROOT = original_amend_desktop
