...
class RAGService:
    ...
    def __init__(self, persist_directory: str = None, use_upgraded_embeddings: bool = False):
        ...
        self._bm25_large_index_skip_logged = False

        print(f"📚 RAG Service initialized with {self.collection_count} chunks")
    ...
    def _get_bm25_results(
        ...
        if n_results <= 0:
            return {}
        if (os.getenv("LEGAL_AI_DISABLE_BM25", "") or "").strip().lower() in {"1", "true", "yes", "on"}:
            return {}
        try:
            bm25_max_chunks = int((os.getenv("LEGAL_AI_BM25_MAX_CHUNKS") or "120000").strip() or "120000")
        except Exception:
            bm25_max_chunks = 120000
        force_large_bm25 = (os.getenv("LEGAL_AI_FORCE_LARGE_BM25", "") or "").strip().lower() in {"1", "true", "yes", "on"}
        if (
            FAST_RAG_MODE
            and not force_large_bm25
            and int(getattr(self, "collection_count", 0) or 0) > max(1, bm25_max_chunks)
        ):
            if not getattr(self, "_bm25_large_index_skip_logged", False):
                print(
                    "[RAG] Skipping BM25 rebuild in fast mode for large index "
                    f"({self.collection_count} chunks > {bm25_max_chunks}); semantic indexed RAG remains active."
                )
                self._bm25_large_index_skip_logged = True
            return {}
